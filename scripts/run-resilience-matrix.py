#!/usr/bin/env python3
"""Run the controlled, resumable resilience campaign recovery experiments."""
...
class RecoveryError(RuntimeError):
    """A fail-closed resilience campaign experiment error."""
...
class RecoveryRunner:
    ...
    def start_observability_forwards(self, evidence: Path) -> None:
        ...
        print("resilience_observability_forwards=ready", flush=True)
    ...
    def baseline_validation(self, evidence: Path) -> None:
        self.validate_to_log("platform-lifecycle.sh", "validate", evidence / "baseline-platform.log")
        self.validate_to_log("observability-lifecycle.sh", "validate", evidence / "baseline-observability.log")
        print("resilience_healthy_baseline=pass", flush=True)

    def restore_baseline(
        self, evidence: Path, component: str, old_uid: str,
        fault_time: float, timeline: list[dict[str, Any]],
    ) -> float:
        print(f"resilience_restoration=started component={component}", flush=True)
        return_code = self.stream_process_with_samples(
            [str(self.root / "scripts" / "platform-lifecycle.sh"), "repair-sessions"],
        ...
        self.validate_to_log("observability-lifecycle.sh", "validate", evidence / "restored-observability.log")
        ...
        restored = time.time()
        print(f"resilience_baseline_restoration=pass component={component}", flush=True)
        return restored
    ...
    def run_attempt(self, component: str, evidence: Path, identity: dict[str, Any]) -> dict[str, Any]:
        ...
        print(
            f"resilience_fault=injecting component={component} pod={before['pod_name']}",
            flush=True,
        )
        ...
        while time.monotonic() < replacement_deadline:
            ...
            if detection_time is None and (
                ...
                detection_time = now
                print(f"resilience_detection=observed component={component}", flush=True)
            ...
            if replacement_ready_time is None and any(pod["ready"] for pod in new_pods):
                ...
                print(f"resilience_replacement=ready component={component}", flush=True)
            if state["service_recovered"] and time.monotonic() <= automatic_deadline:
                automatic_service_recovery_time = now
                print(f"resilience_automatic_service_recovery=observed component={component}", flush=True)
                break
        ...
        self.host_abort_gate()
        print(
            f"resilience_attempt=pass component={component} mode={service_mode} "
    ...
    def run_pilot(self) -> None:
        ...
        try:
            ...
            raise
        print(f"resilience_pilot=pass component={component} raw_evidence={evidence}")
    ...
    def load_or_create_campaign(self, revision: int) -> dict[str, Any]:
        if self.state_path.exists():
            ...
            if (
                ...
                print(f"resilience_campaign={state['campaign_id']} mode=resumed", flush=True)
                return state
            abandoned = self.state_path.with_name(
                f"resilience-campaign-{state.get('campaign_id', 'unknown')}-abandoned.json"
            )
            self.state_path.replace(abandoned)
            print(
                f"resilience_prior_campaign=preserved-abandoned campaign={state.get('campaign_id', 'unknown')}",
                flush=True,
            )
        ...
        print(f"resilience_campaign={campaign_id} mode=resumable", flush=True)
        return state
    ...
    def run_matrix(self) -> None:
        ...
        for repetition in range(1, repetitions + 1):
            for component in self.experiment["controls"]["run_order"]:
                ...
                if key in state["accepted"]:
                    print(
                        f"resilience_condition=already-accepted repetition={repetition} component={component}",
                        flush=True,
                    )
                    continue
                ...
                print(
                    f"resilience_condition=started repetition={repetition} "
                ...
                self.chown_tree(evidence)
                print(
                    f"resilience_condition=pass repetition={repetition} component={component} "
        ...
        print(f"resilience_matrix=pass campaign={state['campaign_id']} accepted=9", flush=True)
    ...
    def emergency_restore(self) -> None:
        if not self.fault_injected or self.baseline_restored:
            return
        print("resilience_emergency_restoration=started", file=sys.stderr, flush=True)
        run(
            [str(self.root / "scripts" / "platform-lifecycle.sh"), "repair-sessions"],
            timeout=1800, check=False,
        )
        run(
            [str(self.root / "scripts" / "observability-lifecycle.sh"), "validate"],
            timeout=1800, check=False,
        )
        print("resilience_emergency_restoration=attempted", file=sys.stderr, flush=True)
...
def main() -> int:
    ...
    try:
        ...
        print(f"error: resilience campaign recovery runner stopped safely: {error}", file=sys.stderr)
        return 1
    return 0
