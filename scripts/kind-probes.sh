...
usage() {
  cat <<'EOF'
Usage: sudo scripts/kind-probes.sh ACTION [--confirm]

Actions:
  preflight-image  Verify the exact Phase 2 base image and target ownership.
  ...
  status-transport Show only the Phase 3 transport probe resources.
  validate-transport
  ...
  status-tun       Show only the Phase 3 TUN probe Pods.
  ...
  status-n6        Show only the Phase 3 N6 routing resources.
...
for variable_name in "${required_variables[@]}"; do
  if [[ -z ${!variable_name:-} ]]; then
    printf 'error: %s is not set in versions/phase-03.env\n' \
      "$variable_name" >&2
    exit 4
  fi
done
...
transport_manifest="$project_root/configs/kubernetes/phase-03/01-transport-probes.yaml"
tun_manifest="$project_root/configs/kubernetes/phase-03/02-tun-capability-probes.yaml"
n6_manifest="$project_root/configs/kubernetes/phase-03/03-n6-routing-probes.yaml"
...
verify_base_image() {
  if ! docker image inspect "$UERANSIM_BASE_IMAGE" >/dev/null 2>&1; then
    printf 'error: exact accepted Phase 2 base image is absent: %s\n' \
...
verify_probe_image() {
  ...
  if [[ $image_id != "$KIND_FEASIBILITY_PROBE_IMAGE_ID" ]]; then
    printf 'error: probe image identity does not match phase-03.env\n' >&2
...
transport_status() {
  kubectl --kubeconfig "$kubeconfig" \
    ...
    --selector app.kubernetes.io/part-of=cn5g-phase-03 \
    -o wide
}
...
tun_status() {
  kubectl --kubeconfig "$kubeconfig" \
    ...
    --selector app.kubernetes.io/part-of=cn5g-phase-03-tun \
    -o wide
}
...
n6_status() {
  kubectl --kubeconfig "$kubeconfig" \
    --namespace "$KIND_FEASIBILITY_NAMESPACE" \
    get pod,service,endpointslice \
    --selector app.kubernetes.io/part-of=cn5g-phase-03-n6 \
    -o wide
}
