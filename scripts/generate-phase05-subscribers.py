#!/usr/bin/env python3
"""Generate deterministic, ignored Phase 5 subscriber and UE material.
...
DEFAULT_PLAN = ROOT / "configs/kubernetes/phase-05/subscriber-plan.json"
DEFAULT_OUTPUT = ROOT / "artifacts/secrets/phase-05"
...
class PlanError(ValueError):
    """Raised when the public subscriber plan violates the Phase 5 contract."""
...
def derive_hex(seed: bytes, purpose: str, imsi: str) -> str:
    payload = f"cn5g-phase05:{purpose}:{imsi}".encode("ascii")
    return hmac.new(seed, payload, hashlib.sha256).digest()[:16].hex().upper()


def object_id(label: str) -> str:
    return hashlib.sha256(f"cn5g-phase05:{label}".encode("ascii")).hexdigest()[:24]


def ue_yaml(plan: dict[str, Any], subscriber: dict[str, Any], seed: bytes) -> str:
    imsi = subscriber["imsi"]
    return f"""# Generated synthetic Phase 5 UE configuration. Do not commit this file.
supi: 'imsi-{imsi}'
mcc: '{plan['plmn']['mcc']}'
mnc: '{plan['plmn']['mnc']}'
protectionScheme: 0
...
def subscriber_document(plan: dict[str, Any], subscriber: dict[str, Any], seed: bytes) -> str:
    ...
    cn5g_managed: {{phase: NumberInt(5), ordinal: NumberInt({ordinal}), dnn: '{dnn}'}}
  }}"""
...
def provisioning_javascript(plan: dict[str, Any], seed: bytes) -> str:
    ...
    return f"""// Generated synthetic Phase 5 provisioning input. Do not commit this file.
...
const managed = open5gs.subscribers.find(
  {{imsi: {{$in: [{imsis}]}}}},
  {{imsi: 1, 'cn5g_managed.phase': 1}}
).toArray();
if (managed.length !== {EXPECTED_SUBSCRIBERS} ||
    managed.some((subscriber) => subscriber.cn5g_managed.phase !== 5)) {{
...
def check_material(plan: dict[str, Any], output: Path) -> None:
    if not output.is_dir() or output.is_symlink():
        raise PlanError("Phase 5 material directory is missing or unsafe")
    ...
    if observed != allowed:
        raise PlanError("Phase 5 material directory has missing or unexpected files")
...
def generate(plan: dict[str, Any], output: Path) -> None:
    if output.exists() and (not output.is_dir() or output.is_symlink()):
        raise PlanError("refusing unsafe Phase 5 output path")
...
def main() -> int:
    ...
    try:
        ...
        if args.validate_plan:
            print("phase05_subscriber_plan_validation=pass")
            return 0
        output = args.output.resolve()
        if args.check:
            if not output.exists():
                print("phase05_subscriber_material_state=absent")
                print("check_result=pass")
                return 0
            check_material(plan, output)
            print("phase05_subscriber_material_state=present-deterministic-and-restricted")
            print("phase05_subscriber_material_validation=pass")
            print("check_result=pass")
            return 0
        generate(plan, output)
        print("phase05_subscriber_material_state=present-deterministic-and-restricted")
        print(f"phase05_subscriber_count={EXPECTED_SUBSCRIBERS}")
        print("phase05_dnn_count=2")
        print("phase05_subscriber_generation=pass")
        return 0
