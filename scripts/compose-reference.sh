...
if ! docker info >/dev/null 2>&1; then
    echo "compose-reference: Docker daemon access is required; use sudo for this host" >&2
    exit 50
fi
...
open5gs-upfd.service'
    ...
    if [ -n "$inactive_units" ]; then
        echo "compose-reference: existing host lab service is not active:" >&2
        echo "$inactive_units" >&2
        return 55
    fi
    echo "host_reference_services=active"
    ...
    for process in nr-gnb nr-ue ns3 waf; do
        ...
        for process_pid in $process_pids; do
            ...
            echo "compose-reference: host process '$process' is already running" >&2
            return 56
        done
    done
    ...
    if [ "$available_kib" -lt "$minimum_kib" ]; then
        echo "compose-reference: at least 12 GiB free is required for the first build" >&2
        return 57
    fi
    ...
    do
        if docker image inspect "$image" >/dev/null 2>&1; then
            ...
            if [ "$owner_url" != "$expected_owner" ] \
                ...
                echo "compose-reference: refusing to replace image tag not owned by this project: $image" >&2
                return 58
            fi
        fi
    done
...
verify_images() {
    ...
    do
        if ! docker image inspect "$image" >/dev/null 2>&1; then
            echo "compose-reference: required image is unavailable: $image" >&2
            return 59
        fi
        ...
        if [ "$owner_url" != "$expected_owner" ]; then
            echo "compose-reference: image ownership URL is invalid: $image" >&2
            return 60
        fi
        ...
        if [ "$image_os" != linux ] || [ "$image_arch" != amd64 ]; then
            echo "compose-reference: unexpected image platform for $image: $image_os/$image_arch" >&2
            return 61
        fi
    ...
    if [ -n "$(docker ps --all --quiet --filter label=com.docker.compose.project="$project")" ]; then
        echo "compose-reference: unexpected project container exists before deployment" >&2
        return 62
    fi
    if [ -n "$(docker network ls --quiet --filter label=com.docker.compose.project="$project")" ]; then
        echo "compose-reference: unexpected project network exists before deployment" >&2
        return 63
    fi
    if [ -n "$(docker volume ls --quiet --filter label=com.docker.compose.project="$project")" ]; then
        echo "compose-reference: unexpected project volume exists before deployment" >&2
        return 64
    fi
...
verify_down() {
    if [ -n "$(docker ps --all --quiet --filter label=com.docker.compose.project="$project")" ]; then
        echo "compose-reference: project container remains after down" >&2
        return 65
    fi
    if [ -n "$(docker network ls --quiet --filter label=com.docker.compose.project="$project")" ]; then
        echo "compose-reference: project network remains after down" >&2
        return 66
    fi
...
cn5g-compose_mongodb-data'
    ...
    if [ "$actual_volumes" != "$expected_volumes" ]; then
        echo "compose-reference: expected only the two persistent MongoDB volumes after down" >&2
        printf 'found:\n%s\n' "$actual_volumes" >&2
        return 67
    fi
...
const marker = {
  _id: 'compose_reference-compose-recreation',
  value: 'synthetic-persistence-evidence'
};
db.cn5g_lifecycle_evidence.replaceOne(
  { _id: marker._id }, marker, { upsert: true }
);
if (!db.cn5g_lifecycle_evidence.findOne(marker)) {
  quit(68);
}
...
if (!db.cn5g_lifecycle_evidence.drop()) {
  quit(70);
}
...
usage() {
    cat <<'EOF'
usage: scripts/compose-reference.sh ACTION [--confirm]
...
case "$action" in
    ...
    status)
        ...
        ;;
    validate)
        "$repository_root/scripts/validate-compose-reference.sh"
        ;;
    logs)
    ...
    destroy)
        if [ "$confirmation" != "--confirm" ]; then
            echo "compose-reference: destroy requires the literal second argument --confirm" >&2
            exit 51
        fi
    ...
    remove-images)
        if [ "$confirmation" != "--confirm" ]; then
            echo "compose-reference: remove-images requires the literal second argument --confirm" >&2
            exit 52
        fi
        if [ -n "$(compose ps --quiet --all)" ]; then
            echo "compose-reference: remove project containers before removing images" >&2
            exit 53
        fi
        for image in \
            cn5g/open5gs:2.7.7 \
