#!/usr/bin/env python3
"""Deterministic repository and workflow policy checks for Phase 9."""
...
VERSION_FILE = ROOT / "versions" / "phase-09.env"
...
def load_versions() -> dict[str, str]:
    ...
    for number, line in enumerate(VERSION_FILE.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if not match:
            raise CheckFailure(f"invalid Phase 9 version assignment at line {number}")
        key, value = match.groups()
        if key in values:
            raise CheckFailure(f"duplicate Phase 9 version key: {key}")
        values[key] = value
    return values
...
def check_workflow() -> None:
    ...
    workflow = Path(os.environ.get("PHASE09_WORKFLOW_PATH", WORKFLOW))
    ...
    expected_actions = {
        "actions/checkout": versions["PHASE_09_CHECKOUT_ACTION_SHA"],
        "actions/setup-python": versions["PHASE_09_SETUP_PYTHON_ACTION_SHA"],
        "actions/upload-artifact": versions["PHASE_09_UPLOAD_ARTIFACT_ACTION_SHA"],
    }
    ...
    for reference in uses:
        ...
        if expected_actions[action] != revision:
            raise CheckFailure(f"action SHA differs from versions/phase-09.env: {action}")
    ...
    print("phase09_workflow_policy=pass permissions=contents-read actions=full-sha")


def check_dockerfiles() -> None:
    dockerfile_root = Path(os.environ.get("PHASE09_DOCKERFILE_ROOT", ROOT / "containers"))
    ...
    print(f"phase09_docker_source_policy=pass dockerfiles={len(dockerfiles)}")
...
def check_structured_data() -> None:
    ...
    for path in repository_files():
        if path.suffix == ".json":
            ...
            yaml_count += 1
    print(f"phase09_structured_data=pass json={json_count} yaml={yaml_count}")
...
def check_markdown_links() -> None:
    ...
    if errors:
        raise CheckFailure("\n".join(errors))
    print(f"phase09_local_links=pass markdown_files={len(markdown)}")
...
def check_privacy() -> None:
    ...
    if findings:
        raise CheckFailure("\n".join(findings))
    print("phase09_publication_boundary=pass private_paths=absent")
