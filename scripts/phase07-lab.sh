...
usage() {
  cat <<'EOF'
Usage: scripts/phase07-lab.sh analyze
       sudo scripts/phase07-lab.sh ACTION

Actions:
  preflight     Validate the accepted Phase 5/6 baseline, host budget,
  ...
  install       Add idle benchmark sidecars and ports, restore the five-UE
                session chain, and validate Phase 5/6 plus the new boundary.
  ...
  analyze       Validate the complete raw campaign and deterministically write
                reviewed CSV, JSON, SVG plots, and the Phase 7 report. No sudo.
  status        Show the exact release state, benchmark Pods, and image state.
  rollback --confirm
                Restore the exact recorded pre-Phase-7 Helm revision and
                revalidate Phase 5/6 without deleting retained data.
...
if [[ $action == analyze ]]; then
  exec "$script_dir/analyze-phase07.py" --project-root "$project_root"
fi
...
# shellcheck source=../versions/phase-07.env
source "$project_root/versions/phase-07.env"

chart="$project_root/charts/cn5g"
phase05_values="$chart/values-phase05.yaml"
phase06_values="$chart/values-phase06.yaml"
phase07_values="$chart/values-phase07.yaml"
experiment="$project_root/benchmarks/phase-07/experiment.json"
...
image_state="$project_root/artifacts/kubernetes/phase-07-image.json"
rollback_state="$project_root/artifacts/kubernetes/phase-07-rollback.json"
raw_root="$project_root/benchmarks/raw/phase-07"
matrix_runner="$script_dir/run-phase07-matrix.py"
matrix_state="$project_root/artifacts/kubernetes/phase-07-campaign.json"
...
require_phase06_baseline() {
  local status values
  status=$(helm --kubeconfig "$kubeconfig" --namespace "$namespace" \
    status "$release" --output json | jq -er '.info.status')
  values=$(release_values)
  [[ $status == deployed && $(jq -r '.phase05.enabled' <<<"$values") == true && \
    $(jq -r '.phase06.enabled' <<<"$values") == true ]] || {
    printf 'error: Phase 7 requires the accepted deployed Phase 5/6 release\n' >&2
    return 1
  }
  printf 'phase06_release=deployed-and-enabled\n'
}
...
verify_resource_budget() {
  ...
  (( memory_kib >= 5 * 1024 * 1024 )) || {
    printf 'error: Phase 7 preparation requires at least 5 GiB available memory\n' >&2
    return 1
  }
  (( disk_kib >= 8 * 1024 * 1024 )) || {
    printf 'error: Phase 7 preparation requires at least 8 GiB free Docker space\n' >&2
    return 1
  }
  printf 'phase07_resource_budget=pass\n'
}

verify_version_contract() {
  [[ $PHASE_07_IMAGE == cn5g/benchmark:0.1.0 && \
    $PHASE_07_BASE_IMAGE_DIGEST =~ ^sha256:[0-9a-f]{64}$ && \
    $PHASE_07_IPERF3_VERSION == 3.19.1-r0 && \
    $PHASE_07_IPERF_PORT == 5201 && $PHASE_07_IPERF_PORT_COUNT == 5 ]] || {
    printf 'error: Phase 7 version contract is invalid\n' >&2
    return 1
  }
  grep -Fq "FROM ${PHASE_07_BASE_IMAGE}@${PHASE_07_BASE_IMAGE_DIGEST}" "$dockerfile"
  grep -Fq "iperf3=${PHASE_07_IPERF3_VERSION}" "$dockerfile"
  grep -Fq "iproute2=${PHASE_07_IPROUTE2_VERSION}" "$dockerfile"
  grep -Fq "iputils=${PHASE_07_IPUTILS_VERSION}" "$dockerfile"
  printf 'phase07_image_source_pin_validation=pass\n'
}
...
PY
  printf 'phase07_experiment_contract=pass levels=1,3,5 repetitions=3\n'
}
...
deterministic_render() {
  ...
  helm lint "$chart" --strict --values "$phase05_values" \
    --values "$phase06_values" --values "$phase07_values"
  for suffix in one two; do
    helm template "$release" "$chart" --namespace "$namespace" \
      --kube-version 1.36.1 --values "$phase05_values" \
      --values "$phase06_values" --values "$phase07_values" \
...
PY
  rm -rf -- "$render_dir"
  trap - RETURN
  printf 'deterministic_phase07_chart_render=pass\n'
}
...
run_preflight() {
  ...
  verify_experiment_contract
  deterministic_render
  printf 'phase07_preflight=pass\n'
  printf 'next_step=sudo ./scripts/phase07-lab.sh build-image\n'
}

write_image_state() {
  local image_id dockerfile_sha state_dir
  image_id=$(docker image inspect --format '{{.Id}}' "$PHASE_07_IMAGE")
  dockerfile_sha=$(sha256sum "$dockerfile" | awk '{print $1}')
  state_dir=${image_state%/*}
  mkdir -p "$state_dir"
  umask 077
  jq -n --arg image "$PHASE_07_IMAGE" --arg image_id "$image_id" \
...
verify_local_image() {
  ...
  [[ -f $image_state && ! -L $image_state && $(stat -c '%a' "$image_state") == 600 ]] || {
    printf 'error: verified Phase 7 image state is absent or unsafe\n' >&2
    return 1
  }
  ...
  current_id=$(docker image inspect --format '{{.Id}}' "$PHASE_07_IMAGE")
  platform=$(docker image inspect --format '{{.Os}}/{{.Architecture}}' "$PHASE_07_IMAGE")
  [[ $state_image == "$PHASE_07_IMAGE" && $state_id == "$current_id" && \
    $expected_sha == "$current_sha" && $platform == linux/amd64 ]] || {
    printf 'error: local benchmark image identity, source, or platform changed\n' >&2
    return 1
  }
  packages=$(docker run --rm --entrypoint /bin/sh "$PHASE_07_IMAGE" -ec \
    "installed=\$(apk info -v 2>/dev/null); \
     printf '%s\\n' \"\$installed\" | grep -Fx 'iperf3-${PHASE_07_IPERF3_VERSION}' >/dev/null; \
     printf '%s\\n' \"\$installed\" | grep -Fx 'iproute2-${PHASE_07_IPROUTE2_VERSION}' >/dev/null; \
     printf '%s\\n' \"\$installed\" | grep -Fx 'iputils-${PHASE_07_IPUTILS_VERSION}' >/dev/null; \
     iperf3 --version | head -n 1; ip -Version")
  printf '%s\n' "$packages"
  printf 'phase07_local_image=verified image_id=%s platform=%s\n' "$current_id" "$platform"
}

build_image() {
  verify_version_contract
  docker build --platform linux/amd64 --pull \
    --file "$dockerfile" --tag "$PHASE_07_IMAGE" "$project_root"
  write_image_state
  verify_local_image
  printf 'phase07_image_build=pass\n'
  printf 'next_step=sudo ./scripts/phase07-lab.sh load-image\n'
}

verify_node_image() {
  local inspect_json labels
  inspect_json=$(docker exec "$node_container" crictl inspecti "$PHASE_07_IMAGE")
  labels=$(jq -r '.info.imageSpec.config.Labels // .info.imageSpec.config.labels // {}' <<<"$inspect_json")
  [[ $(jq -r '.["cn5g.io/phase"] // ""' <<<"$labels") == 07 && \
    $(jq -r '.["cn5g.io/iperf3-version"] // ""' <<<"$labels") == "$PHASE_07_IPERF3_VERSION" ]] || {
    printf 'error: kind runtime benchmark image labels are unexpected\n' >&2
    return 1
  }
  printf 'phase07_node_image=verified\n'
}

load_image() {
  require_cluster
  verify_local_image
  kind load docker-image --name "$KIND_CLUSTER_NAME" "$PHASE_07_IMAGE"
  verify_node_image
  printf 'phase07_image_load=pass\n'
  printf 'next_step=sudo ./scripts/phase07-lab.sh install\n'
}
...
record_rollback_state() {
  ...
  if [[ -f $rollback_state ]]; then
    [[ ! -L $rollback_state && $(stat -c '%a' "$rollback_state") == 600 ]] || {
      printf 'error: Phase 7 rollback state is unsafe\n' >&2
      return 1
    }
    printf 'phase07_rollback_state=already-recorded\n'
    return
  fi
  ...
  printf 'phase07_rollback_state=recorded baseline_revision=%s\n' "$revision"
}
...
verify_benchmark_workloads() {
  ...
  printf 'phase07_benchmark_boundary=pass ue_clients=5 dnn_servers=2 ports_per_server=5 capabilities=none\n'
}
...
install_overlay() {
  ...
  status=$(helm --kubeconfig "$kubeconfig" --namespace "$namespace" \
    status "$release" --output json | jq -er '.info.status')
  enabled=$(release_values | jq -r '.phase07.enabled // false')
  [[ $status == deployed && $enabled =~ ^(true|false)$ ]] || {
    printf 'error: release state is outside the Phase 7 contract\n' >&2
    return 1
  }
  record_rollback_state
  helm upgrade "$release" "$chart" --kubeconfig "$kubeconfig" \
    --namespace "$namespace" --values "$phase05_values" \
    --values "$phase06_values" --values "$phase07_values" \
    --set-string "images.benchmark.expectedImageID=$image_id" \
    --dry-run=server >/dev/null
  printf 'server_side_phase07_dry_run=pass\n'
  helm upgrade "$release" "$chart" --kubeconfig "$kubeconfig" \
    --namespace "$namespace" --values "$phase05_values" \
    --values "$phase06_values" --values "$phase07_values" \
    --set-string "images.benchmark.expectedImageID=$image_id" \
  ...
  "$script_dir/phase05-lab.sh" repair-sessions
  "$script_dir/phase06-lab.sh" validate
  verify_benchmark_workloads
  printf 'phase07_install=pass\n'
  printf 'next_step=sudo ./scripts/phase07-lab.sh pilot\n'
}
...
wait_for_ue_replicas() {
  ...
  for attempt in $(seq 1 300); do
    ...
    ' <<<"$object")
    [[ $observed == true ]] && {
      printf 'phase07_ue_statefulset=converged replicas=%s\n' "$expected"
      return 0
    }
    sleep 2
  done
...
run_pilot() {
  ...
  require_phase06_baseline
  values=$(release_values)
  [[ $(jq -r '.phase07.enabled // false' <<<"$values") == true ]] || {
    printf 'error: install the Phase 7 overlay before the pilot\n' >&2
    return 1
  }
  verify_local_image
  verify_benchmark_workloads
  ...
  "$script_dir/phase05-lab.sh" repair-sessions
  printf 'phase07_pilot_session_state=clean benchmark_servers=clean\n'
  ...
  if [[ $route != *"from $ue_ip dev uesimtun0 table 1000"* || \
    ...
    "${kubectl_ns[@]}" exec cn5g-ue-0 -c benchmark-client -- \
      iperf3 --client "$endpoint_ip" --port "$PHASE_07_IPERF_PORT" \
...
PY
  ...
  if ! "$script_dir/phase05-lab.sh" validate; then
    printf 'error: five-UE baseline did not recover; run phase05 repair-sessions\n' >&2
    pilot_result=1
  fi
  (( pilot_result == 0 )) || {
    printf 'phase07_pilot=fail raw_evidence=%s\n' "$run_dir" >&2
    return 1
  }
  printf 'phase07_route_enforcement=pass device=uesimtun0\n'
  printf 'phase07_pilot=pass raw_evidence=%s\n' "$run_dir"
  printf 'next_step=sudo ./scripts/phase07-lab.sh run-matrix\n'
}
...
run_matrix() {
  ...
  if ! python3 "$matrix_runner" \
    ...
    printf 'error: Phase 7 matrix stopped; rerun the same command after reviewing the retained failed attempt\n' >&2
    return 1
  fi
  "$script_dir/phase05-lab.sh" validate
  "$script_dir/phase06-lab.sh" validate
  printf 'phase07_matrix_validation=pass\n'
...
rollback_overlay() {
  ...
  [[ -f $rollback_state && ! -L $rollback_state && $(stat -c '%a' "$rollback_state") == 600 ]] || {
    printf 'error: Phase 7 rollback state is absent or unsafe\n' >&2
    return 1
  }
  ...
  "$script_dir/phase06-lab.sh" validate
  rm -- "$rollback_state"
  printf 'phase07_rollback=pass target_revision=%s mongodb_pvc_identity=preserved\n' "$baseline"
}
...
show_status() {
  ...
  release_values | jq '{phase05:.phase05.enabled,phase06:.phase06.enabled,phase07:(.phase07.enabled // false)}'
  ...
  if [[ -f $image_state ]]; then verify_local_image; else printf 'phase07_local_image_state=absent\n'; fi
}
