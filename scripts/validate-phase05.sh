...
phase05_enabled=$(helm --kubeconfig "$kubeconfig" --namespace "$namespace" \
  get values "$release" --all --output json | jq -er '.phase05.enabled')
if [[ $phase05_enabled != "true" ]]; then
  printf 'error: deployed release is not using the Phase 5 topology\n' >&2
  exit 20
fi
printf 'helm_release_status=deployed\nphase05_topology=enabled\n'
...
subscriber_count=$("${kubectl_namespace[@]}" exec \
  ...
  open5gs --eval 'db.subscribers.countDocuments({"cn5g_managed.phase":5})')
...
printf 'phase05_validation=pass\n'
