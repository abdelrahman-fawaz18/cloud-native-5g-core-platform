...
usage() {
  cat <<'EOF'
Usage: sudo scripts/observability-lifecycle.sh ACTION

Actions:
  preflight       Validate platform, the resource budget, chart, image pins,
                  and deterministic rendering. The platform validator may
  ...
  validate        Revalidate platform plus Prometheus, Grafana, Loki, Alloy,
                  Kubernetes/telecom metrics, reviewed performance and recovery results,
  ...
  reset-stale-state --confirm
                  Archive observability stack checkpoints from a deleted cluster only
                  when the replacement release is a different platform
  ...
  destroy --confirm
                  After uninstall, remove only verified observability stack PVCs, the
                  Grafana Secret, and the empty observability namespace.

The helper never deletes the kind cluster, MongoDB PVC, platform subscriber
...
# shellcheck source=../versions/observability-runtime.env
source "$project_root/versions/observability-runtime.env"

core_chart="$project_root/charts/cn5g"
core_default_profile="$project_root/profiles/default.yaml"
core_only_profile="$project_root/profiles/core-only.yaml"
...
credential_dir="$project_root/artifacts/secrets/observability"
admin_user_file="$credential_dir/admin-user"
admin_password_file="$credential_dir/admin-password"
state_file="$project_root/artifacts/kubernetes/observability.state"
grafana_soak_state_file="$project_root/artifacts/kubernetes/observability-grafana-soak.json"
hardening_state_file="$project_root/artifacts/kubernetes/observability-dashboard-hardening.json"
...
verify_namespace_ownership() {
  local namespace_json part domain
  ...
  domain=$(jq -r '.metadata.labels["cn5g.io/domain"] // ""' <<<"$namespace_json")
  if [[ $part != cn5g-platform || $domain != observability ]]; then
...
verify_platform_release() {
  ...
  enabled=$(helm --kubeconfig "$kubeconfig" --namespace "$core_namespace" \
    get values "$core_release" --all --output json | jq -er '.platform.enabled')
  if [[ $status != deployed || $enabled != true ]]; then
    printf 'error: observability stack requires the accepted deployed platform topology\n' >&2
    return 1
  fi
  printf 'platform_release=deployed-and-enabled\n'
}
...
verify_resource_budget() {
  ...
  if (( memory_kib < 4 * 1024 * 1024 )); then
    printf 'error: observability stack requires at least 4 GiB available memory\n' >&2
    return 1
  fi
  if (( disk_kib < 6 * 1024 * 1024 )); then
    printf 'error: observability stack requires at least 6 GiB on the Docker filesystem\n' >&2
    return 1
  fi
  printf 'observability_resource_budget=pass\n'
}
...
verify_image_pins() {
  ...
  for variable in PROMETHEUS GRAFANA LOKI ALLOY KUBE_STATE_METRICS UE_PROBE_PYTHON; do
    ...
    [[ $image != *:latest && $digest =~ ^sha256:[0-9a-f]{64}$ ]] || {
      printf 'error: invalid observability stack image pin: %s\n' "$variable" >&2
      return 1
    }
    printf 'image=%s@%s identity=pinned\n' "$image" "$digest"
  done
  printf 'observability_image_pin_validation=pass\n'
}

deterministic_render() {
  local render_dir cleanup
  "$project_root/scripts/generate-performance-dashboard-metrics.py" --check
  "$project_root/scripts/generate-resilience-dashboard-metrics.py" --check
  render_dir=$(mktemp -d)
  printf -v cleanup 'rm -rf -- %q' "$render_dir"
  trap "$cleanup" RETURN
  helm lint "$core_chart" --strict --values "$core_default_profile"
  helm lint "$observability_chart" --strict
  helm template "$core_release" "$core_chart" --namespace "$core_namespace" \
    --kube-version 1.36.1 --values "$core_default_profile" \
    > "$render_dir/core-one.yaml"
  helm template "$core_release" "$core_chart" --namespace "$core_namespace" \
    --kube-version 1.36.1 --values "$core_default_profile" \
    > "$render_dir/core-two.yaml"
...
PY
  rm -rf -- "$render_dir"
  trap - RETURN
  printf 'deterministic_observability_chart_render=pass\n'
}

run_preflight() {
  local platform_output platform_result
  require_cluster
  verify_resource_budget
  verify_platform_release
  if platform_output=$("$script_dir/platform-lifecycle.sh" validate 2>&1); then
    printf '%s\n' "$platform_output"
  else
    platform_result=$?
    printf '%s\n' "$platform_output"
    if [[ $platform_output == *"curl: (28)"* || \
          $platform_output == *"UPF does not expose five complete PFCP/GTP-U sessions"* ]]; then
      printf 'error: platform session state is stale; run the scoped recovery and retry:\n' >&2
      printf '  sudo ./scripts/platform-lifecycle.sh repair-sessions\n' >&2
    fi
    return "$platform_result"
  fi
  verify_image_pins
  deterministic_render
  printf 'observability_preflight=pass\n'
}
...
verify_secret() {
  ...
  [[ $(jq -r '.metadata.labels["app.kubernetes.io/managed-by"] // ""' <<<"$secret_json") == cn5g-observability-lab ]] || {
...
prepare_secret() {
  ...
  if ! kubectl --kubeconfig "$kubeconfig" get namespace "$namespace" >/dev/null 2>&1; then
    ...
    kubectl --kubeconfig "$kubeconfig" label namespace "$namespace" \
      app.kubernetes.io/part-of=cn5g-platform cn5g.io/domain=observability
  fi
  verify_namespace_ownership
  if ! "${obs_kubectl[@]}" get secret "$secret_name" >/dev/null 2>&1; then
    ...
    "${obs_kubectl[@]}" label secret "$secret_name" \
      ...
      app.kubernetes.io/managed-by=cn5g-observability-lab \
      cn5g.io/domain=observability
  fi
  verify_secret
  printf 'observability_secret_preparation=pass\n'
}
...
reset_stale_state() {
  ...
  [[ -f $state_file && ! -L $state_file ]] || {
    printf 'error: observability stack base checkpoint is absent or unsafe\n' >&2
    return 1
  }
  local mode saved_revision current_revision observability_enabled timestamp file archive
  mode=$(stat -c '%a' "$state_file")
  [[ $mode == 600 ]] || {
    printf 'error: observability stack base checkpoint must use mode 600\n' >&2
    return 1
  }
  saved_revision=$(awk -F= '$1 == "BASE_CORE_REVISION" && $2 ~ /^[0-9]+$/ {print $2}' "$state_file")
  [[ -n $saved_revision ]] || {
    printf 'error: observability stack base checkpoint is invalid\n' >&2
    return 1
  }
  current_revision=$(helm --kubeconfig "$kubeconfig" --namespace "$core_namespace" \
    status "$core_release" --output json | jq -er '.version')
  observability_enabled=$(helm --kubeconfig "$kubeconfig" --namespace "$core_namespace" \
    get values "$core_release" --all --output json | jq -r '.observability.enabled // false')
  [[ $observability_enabled == false ]] || {
    printf 'error: stale-state reset requires the deployed platform topology\n' >&2
    return 1
  }
  ...
  for file in "$hardening_state_file" "$grafana_soak_state_file"; do
    if [[ -e $file || -L $file ]]; then
      [[ -f $file && ! -L $file && $(stat -c '%a' "$file") == 600 ]] || {
        printf 'error: observability stack checkpoint is unsafe: %s\n' "$file" >&2
        return 1
      }
      jq -e 'type == "object"' "$file" >/dev/null || {
        printf 'error: observability stack JSON checkpoint is invalid: %s\n' "$file" >&2
        return 1
      }
    fi
  done
  ...
  printf 'observability_stale_state=archived old_revision=%s current_revision=%s\n' \
...
server_dry_run() {
  ...
  rm -rf -- "$render_dir"
  trap - RETURN
  printf 'server_side_observability_dry_run=pass\n'
}
...
install_observability() {
  local core_revision observability_enabled
  require_cluster
  verify_resource_budget
  verify_platform_release
  verify_secret
  ...
  if [[ $observability_enabled == true ]]; then
    printf 'observability_core_overlay=already-active upgrade=skipped\n'
  else
    helm upgrade "$core_release" "$core_chart" --kubeconfig "$kubeconfig" \
      --namespace "$core_namespace" --values "$core_default_profile" \
      --wait=watcher --timeout 15m
  fi
  "${core_kubectl[@]}" rollout status statefulset/cn5g-ue --timeout=600s
  printf 'observability_ue_probe_rollout=pass replicas=5\n'
  ...
  printf 'observability_install=pass\n'
}
...
validate_prometheus() {
  ...
  jq -e '
    ...
    ([.data.activeTargets[] | select(.labels.job == "performance-reviewed-results")] | length) == 1 and
    ([.data.activeTargets[] | select(.labels.job == "resilience-reviewed-results")] | length) == 1 and
    ([.data.activeTargets[] | select(.labels.job | test("^(alert-exercise|performance-reviewed-results|resilience-reviewed-results|open5gs-(amf|pcf|smf|upf)|cn5g-ue-user-plane|kube-state-metrics|kubernetes-node|kubernetes-cadvisor)$")) | .health] | all(. == "up"))
  ...
  for query_expected in \
    'cn5g_performance_reviewed_campaign_info{status="reviewed_complete"}:1' \
    'cn5g_performance_reviewed_accepted_conditions:9' \
    'cn5g_performance_reviewed_repetitions:3' \
    'count(cn5g_performance_reviewed_procedure_success_ratio):6'; do
    query=${query_expected%:*}
    expected=${query_expected##*:}
    query_json=$(prometheus_query "$query")
    value=$(jq -er '.data.result | if length == 1 then .[0].value[1] else error("unexpected reviewed-result vector") end' <<<"$query_json")
    awk -v observed="$value" -v expected="$expected" 'BEGIN {exit !(observed+0 == expected+0)}' || {
      printf 'error: reviewed performance campaign metric did not meet contract: %s observed=%s\n' "$query" "$value" >&2
      return 1
    }
  done
  query_json=$(prometheus_query 'count({__name__=~"cn5g_performance_reviewed_.+"})')
  reviewed_series_count=$(jq -er '
    .data.result |
    if length == 1 then .[0].value[1] else error("missing reviewed performance campaign series count") end
  ' <<<"$query_json")
  if ! awk -v observed="$reviewed_series_count" 'BEGIN {exit !(observed+0 == 556 && observed+0 <= 600)}'; then
    printf 'error: reviewed performance campaign metric cardinality does not match the generated contract: %s\n' "$reviewed_series_count" >&2
    return 1
  fi
  printf 'performance_reviewed_metric_validation=pass accepted_conditions=9 series=%s limit=600\n' \
    "$reviewed_series_count"
  for query_expected in \
    'cn5g_resilience_reviewed_campaign_info{status="reviewed_complete"}:1' \
    'cn5g_resilience_reviewed_accepted_conditions:9' \
    'cn5g_resilience_reviewed_repetitions:3' \
    'sum(cn5g_resilience_reviewed_recovery_outcomes_total{mode="automatic"}):0' \
    'sum(cn5g_resilience_reviewed_recovery_outcomes_total{mode="operator_assisted"}):9' \
    'cn5g_resilience_reviewed_baseline_restorations_total:9' \
    'cn5g_resilience_reviewed_mongodb_pvc_preserved:1'; do
    query=${query_expected%:*}
    expected=${query_expected##*:}
    query_json=$(prometheus_query "$query")
    value=$(jq -er '.data.result | if length == 1 then .[0].value[1] else error("unexpected resilience campaign reviewed-result vector") end' <<<"$query_json")
    awk -v observed="$value" -v expected="$expected" 'BEGIN {exit !(observed+0 == expected+0)}' || {
      printf 'error: reviewed resilience campaign metric did not meet contract: %s observed=%s\n' "$query" "$value" >&2
      return 1
    }
  done
  query_json=$(prometheus_query 'count({__name__=~"cn5g_resilience_reviewed_.+"})')
  recovery_series_count=$(jq -er '
    .data.result |
    if length == 1 then .[0].value[1] else error("missing reviewed resilience campaign series count") end
  ' <<<"$query_json")
  if ! awk -v observed="$recovery_series_count" 'BEGIN {exit !(observed+0 == 75 && observed+0 <= 100)}'; then
    printf 'error: reviewed resilience campaign metric cardinality does not match the generated contract: %s\n' "$recovery_series_count" >&2
    return 1
  fi
  printf 'resilience_reviewed_metric_validation=pass accepted_conditions=9 series=%s limit=100\n' \
...
validate_observability() {
  ...
  stop_forward
  trap - RETURN
  printf 'observability_validation=pass\n'
}
...
test_alerts() {
  ...
  stop_forward
  trap - RETURN
  printf 'observability_alert_lifecycle=pass tested=3\n'
}
...
uninstall_observability() {
  ...
  helm upgrade "$core_release" "$core_chart" --kubeconfig "$kubeconfig" \
    --namespace "$core_namespace" --values "$core_only_profile" \
    --wait=watcher --timeout 15m
  "$script_dir/platform-lifecycle.sh" validate
  if [[ -f $state_file && ! -L $state_file ]]; then
    ...
    rm -f -- "$state_file"
    printf 'observability_base_core_revision=%s state=removed\n' "$base_revision"
  fi
  if [[ -f $grafana_soak_state_file && ! -L $grafana_soak_state_file ]]; then
    rm -f -- "$grafana_soak_state_file"
  fi
  printf 'observability_uninstall=pass persistent_data=retained\n'
}
...
destroy_observability() {
  ...
  printf 'observability_destroy=pass scoped_data_and_credentials=removed\n'
}

case "$action" in
  preflight) run_preflight ;;
  prepare-secret) prepare_secret ;;
  install) install_observability ;;
  validate) validate_observability ;;
  ...
  status)
    ...
    ;;
  uninstall) uninstall_observability ;;
  destroy) destroy_observability ;;
esac
