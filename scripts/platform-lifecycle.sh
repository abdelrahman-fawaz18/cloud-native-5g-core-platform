...
usage() {
  cat <<'EOF'
Usage: sudo scripts/platform-lifecycle.sh ACTION

Actions:
  preflight       Read-only cluster/resource, identity-plan, Secret-material,
                  and deterministic Helm-render checks.
  prepare-secret  Create or verify the exact file-backed platform Secret.
  install         Install the accepted five-UE/two-DNN platform directly on a
                  prepared cluster, then converge and validate all sessions.
  ...
  repair-sessions Quiesce the five UEs, restart only the project-owned 5G
                  session chain in dependency order, and revalidate platform.
  ...
  validate        Reconcile the two exact kind-node return routes and run the
                  complete per-UE platform validator.
  ...
  rollback        Restore the saved single-UE profile revision, remove only four
                  platform-managed subscriber records, and validate single-UE profile.
  status          Show the Helm release and platform namespace-scoped objects.
  reset-stale-state --confirm
                  Preserve and archive a rollback state from a deleted cluster
                  only when the current release is a different single-UE profile/PVC
                  lineage. This never changes the live release or database.
  remove-secret --confirm
                  Remove only the verified platform Secret after rollback.

The helper never deletes the MongoDB claim, kind cluster, single-UE profile Secret, or
unrelated resources. Cluster lifecycle remains owned by cluster-lifecycle.sh.
EOF
}

action=${1:-}
case "$action" in
  preflight|prepare-secret|install|upgrade|repair-sessions|validate|test-invalid-ue|test-reprovision|observe-resources|rollback|status) ;;
...
profile_name=${CN5G_PROFILE:-default}
case "$profile_name" in
  default|core-only|resource-limited) ;;
  *)
    printf 'error: unsupported full-platform profile: %s\n' "$profile_name" >&2
    exit 2
    ;;
esac
...
profile="$project_root/profiles/${profile_name}.yaml"
plan="$project_root/configs/kubernetes/platform/subscriber-plan.json"
material_dir="$project_root/artifacts/secrets/platform"
kubeconfig="$project_root/artifacts/kubernetes/cn5g.kubeconfig"
state_file="$project_root/artifacts/kubernetes/platform-upgrade.state"
secret_name=cn5g-subscribers
namespace=$CN5G_KUBERNETES_NAMESPACE
release=$CN5G_HELM_RELEASE_NAME
node_container="${KIND_CLUSTER_NAME}-control-plane"
generator="$script_dir/generate-subscribers.py"
validator="$script_dir/validate-platform.sh"
invalid_ue_manifest="$project_root/configs/kubernetes/platform/invalid-ue-pod.yaml"
reprovision_manifest="$project_root/configs/kubernetes/platform/reprovision-job.yaml"

for required_file in "$chart/Chart.yaml" "$chart/values.yaml" "$profile" \
...
verify_resource_budget() {
  ...
  if (( memory_kib < 6 * 1024 * 1024 )); then
    printf 'error: platform requires at least 6 GiB available memory before rollout\n' >&2
    return 1
  fi
  if (( swap_kib < 512 * 1024 )); then
    printf 'error: platform requires at least 512 MiB configured swap\n' >&2
    return 1
  fi
  if (( disk_kib < 6 * 1024 * 1024 )); then
    printf 'error: platform requires at least 6 GiB on the Docker filesystem\n' >&2
    return 1
  fi
  printf 'platform_resource_budget=pass\n'
}
...
verify_secret_ownership() {
  ...
  if [[ $instance != "$release" || $part != "cn5g-core" || \
        $managed != "cn5g-platform" ]]; then
    printf 'error: platform Secret ownership labels are unexpected\n' >&2
    return 1
  fi
}
...
verify_secret() {
  ...
  verify_secret_ownership
  secret_json=$("${kubectl_namespace[@]}" get secret "$secret_name" --output json)
  expected_keys=$(platform_secret_files | awk -F/ '{print $NF}' | sort)
  observed_keys=$(jq -r '.data | keys[]' <<<"$secret_json" | sort)
  if [[ $observed_keys != "$expected_keys" ]]; then
    printf 'error: platform Secret key set differs from the generated contract\n' >&2
    return 1
  fi
  while IFS= read -r file; do
    ...
    if [[ $local_hash != "$cluster_hash" ]]; then
      printf 'error: platform Secret content mismatch: %s\n' "$name" >&2
      return 1
    fi
  done < <(platform_secret_files)
  printf 'platform_subscriber_secret=present-matching-and-project-owned\n'
}
...
prepare_secret() {
  ...
  verify_secret
  printf 'platform_secret_preparation=pass\n'
}
...
reconcile_return_routes() {
  ...
  for subnet_metric in "10.60.0.0/24:46060" "10.61.0.0/24:46061"; do
    ...
    if [[ $existing != "$prefix "* ]] || ! recognized_return_route "$existing" "$subnet" "$metric"; then
      printf 'error: platform return route did not converge: %s\n' "$subnet" >&2
      return 1
    fi
    printf 'kind_node_return_route=%s\n' "$existing"
  done
  printf 'platform_return_routes=pass\n'
}
...
wait_for_platform_foundation() {
  ...
  "${kubectl_namespace[@]}" rollout status "statefulset/${release}-mongodb" \
    --timeout=240s
  printf 'platform_foundation_readiness=pass\n'
}
...
scale_platform_ues() {
  ...
  if [[ $replicas == "0" ]]; then
    ...
    printf 'platform_ue_statefulset=quiesced\n'
    return
  fi
  "${kubectl_namespace[@]}" rollout status "statefulset/${release}-ue" \
    --timeout=360s
  printf 'platform_ue_statefulset=ready replicas=%s\n' "$replicas"
}

wait_for_platform_nrf_profiles() {
  ...
  for attempt in $(seq 1 60); do
    ...
    if [[ $count == "9" ]]; then
      printf 'platform_nrf_profile_convergence=pass count=9\n'
      return
    fi
    sleep 2
  done
...
restart_session_chain() {
  ...
  wait_for_platform_nrf_profiles
  scale_platform_ues 5
  printf 'statefulset=ue session_cache_reset=pass\n'
}

repair_platform_sessions() {
  verify_release_deployed
  if [[ $(release_platform_enabled) != "true" ]]; then
    printf 'error: session repair requires the deployed platform topology\n' >&2
    return 1
  fi
  verify_secret
  printf 'platform_session_repair=dependency-ordered-restart\n'
  restart_session_chain
  reconcile_return_routes
  "$validator"
  printf 'platform_session_repair=pass\n'
}
...
read_state() {
  if [[ ! -f $state_file || -L $state_file || $(stat -c '%a' "$state_file") != "600" ]]; then
    printf 'error: platform rollback state is absent or unsafe\n' >&2
    return 1
  fi
  # shellcheck disable=SC1090
  source "$state_file"
  if [[ ! ${BASE_REVISION:-} =~ ^[1-9][0-9]*$ || -z ${PVC_UID:-} || -z ${PVC_VOLUME:-} ]]; then
    printf 'error: platform rollback state is invalid\n' >&2
    return 1
  fi
}
...
verify_pvc_identity() {
  ...
  if [[ $uid != "$PVC_UID" || $volume != "$PVC_VOLUME" ]]; then
    printf 'error: MongoDB PVC identity changed across platform lifecycle\n' >&2
    return 1
  fi
...
reset_stale_state() {
  verify_release_deployed
  if [[ $(release_platform_enabled) != "false" ]]; then
    printf 'error: stale-state reset requires the deployed single-UE profile topology\n' >&2
    return 1
  fi
  read_state
  ...
  chmod 0600 "$archive"
  printf 'platform_stale_state=archived old_revision=%s current_revision=%s\n' \
    "$BASE_REVISION" "$current_revision"
}

controlled_upgrade() {
  local current_revision current_platform current_status new_revision
  verify_secret
  current_revision=$(release_json | jq -er '.version')
  current_status=$(release_json | jq -er '.info.status')
  current_platform=$(release_platform_enabled)
  if [[ ! -e $state_file ]]; then
    if [[ $current_status != "deployed" || $current_platform != "false" ]]; then
      printf 'error: a new platform migration requires the deployed single-UE profile topology\n' >&2
      printf 'release_status=%s platform_enabled=%s\n' \
        "$current_status" "$current_platform" >&2
      return 1
    fi
    ...
    printf 'single_ue_ue_deployment=scaled-to-zero-for-kind-migration\n'
  else
    read_state
    verify_pvc_identity
    case "$current_status" in
      ...
      *)
        printf 'error: release state is not safe for controlled platform resume: %s\n' \
          "$current_status" >&2
        return 1
        ;;
    esac
    case "$current_platform" in
      false)
        ...
        printf 'controlled_platform_upgrade_resume=pre-apply\n'
        ;;
      true)
        printf 'controlled_platform_upgrade_resume=post-apply-convergence\n'
        ;;
      *)
        printf 'error: platform enablement value is invalid: %s\n' \
          "$current_platform" >&2
        return 1
        ;;
    esac
  ...
  helm upgrade "$release" "$chart" --kubeconfig "$kubeconfig" \
    --namespace "$namespace" --values "$profile" --dry-run=server \
    --hide-secret >/dev/null
  printf 'server_side_platform_upgrade_dry_run=pass\n'
  helm upgrade "$release" "$chart" --kubeconfig "$kubeconfig" \
    --namespace "$namespace" --values "$profile"
  printf 'helm_platform_upgrade_submission=pass\n'
  ...
  scale_platform_ues 0
  wait_for_platform_foundation "$new_revision"
  restart_session_chain
  printf 'platform_workload_readiness=pass\n'
  reconcile_return_routes
  verify_pvc_identity
  "$validator"
  printf 'platform_upgrade_revision=%s\nplatform_upgrade=pass\n' "$new_revision"
}

install_platform() {
  local installed_revision status enabled
  verify_secret
  if helm --kubeconfig "$kubeconfig" --namespace "$namespace" \
      status "$release" >/dev/null 2>&1; then
    status=$(release_json | jq -er '.info.status')
    enabled=$(release_platform_enabled)
    if [[ $status == deployed && $enabled == true ]]; then
      reconcile_return_routes
      "$validator"
      printf 'platform_install=already-active\n'
      return
    fi
    printf 'error: an existing release is not the accepted full-platform topology\n' >&2
    printf 'release_status=%s platform_enabled=%s\n' "$status" "$enabled" >&2
    return 1
  fi
  helm install "$release" "$chart" --kubeconfig "$kubeconfig" \
    --namespace "$namespace" --values "$profile" --dry-run=server \
    --hide-secret >/dev/null
  printf 'server_side_platform_install_dry_run=pass\n'
  helm install "$release" "$chart" --kubeconfig "$kubeconfig" \
    --namespace "$namespace" --values "$profile"
  installed_revision=$(release_json | jq -er '.version')
  scale_platform_ues 0
  wait_for_platform_foundation "$installed_revision"
  restart_session_chain
  reconcile_return_routes
  "$validator"
  printf 'platform_install=pass revision=%s\n' "$installed_revision"
}
...
cleanup_invalid_ue() {
  local kind name=cn5g-platform-invalid-ue
...
invalid_ue_experiment_body() {
  ...
  "${kubectl_namespace[@]}" create secret generic cn5g-platform-invalid-ue \
    ...
    --dry-run=client --output json | jq \
      --arg instance "$release" '
        .metadata.labels = {
          ...
          --field-manager=cn5g-platform --filename=- || return 1
  "${kubectl_namespace[@]}" apply --server-side \
    --field-manager=cn5g-platform --filename "$invalid_ue_manifest" || return 1
  for attempt in $(seq 1 45); do
    logs=$("${kubectl_namespace[@]}" logs cn5g-platform-invalid-ue \
  ...
  managed_count=$("${kubectl_namespace[@]}" exec \
    "statefulset/${release}-mongodb" --container mongodb -- \
    mongosh --quiet open5gs --eval \
    'db.subscribers.countDocuments({"cn5g_managed.topology":"multi-ue"})') || return 1
...
test_invalid_ue() {
  ...
  verify_release_deployed
  if [[ $(release_platform_enabled) != "true" ]]; then
    printf 'error: invalid-UE test requires the platform topology\n' >&2
    return 1
  fi
  cleanup_invalid_ue
  temp_dir=$(mktemp -d)
...
cleanup_reprovision_job() {
  local name=cn5g-platform-reprovision
...
wait_for_reprovision_job() {
  local name=cn5g-platform-reprovision attempt job_json complete failed reasons
...
reprovision_experiment_body() {
  ...
  removed=$("${kubectl_namespace[@]}" exec \
    "statefulset/${release}-mongodb" --container mongodb -- \
    mongosh --quiet open5gs --eval \
    "db.subscribers.deleteOne({imsi:'${target_imsi}', 'cn5g_managed.topology':'multi-ue'}).deletedCount") \
    || return 1
  case "$removed" in
  ...
  "${kubectl_namespace[@]}" apply --server-side \
    --field-manager=cn5g-platform --filename "$reprovision_manifest" || return 1
  wait_for_reprovision_job || return 1
  result=$("${kubectl_namespace[@]}" logs job/cn5g-platform-reprovision \
...
test_reprovision() {
  ...
  verify_release_deployed
  if [[ $(release_platform_enabled) != "true" ]]; then
    printf 'error: reprovision test requires the platform topology\n' >&2
    return 1
  fi
  cleanup_reprovision_job
...
observe_platform_resources() {
  ...
  verify_release_deployed
  if [[ $(release_platform_enabled) != "true" ]]; then
    printf 'error: resource observation requires the platform topology\n' >&2
    return 1
  fi
  "$validator"
  ...
  for index in "${!targets[@]}"; do
    sample=$(platform_cgroup_sample "${targets[$index]}" "${containers[$index]}")
  ...
  for index in "${!targets[@]}"; do
    ...
    contract=$(platform_resource_contract \
...
cleanup_platform_subscribers() {
  local result
  result=$("${kubectl_namespace[@]}" exec "statefulset/${release}-mongodb" \
    --container mongodb -- mongosh --quiet open5gs --eval '
      const managed = db.subscribers.countDocuments({"cn5g_managed.topology":"multi-ue"});
      const total = db.subscribers.countDocuments({});
      if (managed === 4 && total === 5) {
        const removed = db.subscribers.deleteMany({"cn5g_managed.topology":"multi-ue"});
        if (removed.deletedCount !== 4) throw new Error(`expected four platform-only records, removed ${removed.deletedCount}`);
        if (db.subscribers.countDocuments({}) !== 1) throw new Error("expected one restored single-UE profile subscriber");
        print("platform_subscriber_cleanup=pass removed=4 remaining=1");
      } else if (managed === 0 && total === 1) {
        print("platform_subscriber_cleanup=pass removed=0 remaining=1 state=already-clean");
      } else {
        throw new Error(`unexpected subscriber cleanup state managed=${managed} total=${total}`);
      }')
  if [[ $result != "platform_subscriber_cleanup=pass removed=4 remaining=1" && \
        $result != "platform_subscriber_cleanup=pass removed=0 remaining=1 state=already-clean" ]]; then
    printf 'error: platform subscriber cleanup returned an unexpected result\n' >&2
    return 1
  fi
  printf '%s\n' "$result"
}
...
controlled_rollback() {
  local current_platform rollback_revision status description subscriber_job
  read_state
  verify_pvc_identity
  current_platform=$(release_platform_enabled)
  case "$current_platform" in
    true)
      ...
      printf 'controlled_platform_rollback=post-apply revision=%s\n' \
        "$rollback_revision"
      ;;
    false)
      status=$(release_json | jq -er '.info.status')
      ...
      if [[ $status != "deployed" || \
        ...
        printf 'error: single-UE profile release is not the recorded rollback target\n' >&2
        printf 'status=%s description=%s\n' "$status" "$description" >&2
        return 1
      fi
      remove_enterprise_return_route
      printf 'controlled_platform_rollback_resume=post-apply revision=%s\n' \
        "$rollback_revision"
      ;;
    *)
      printf 'error: platform enablement value is invalid during rollback\n' >&2
      return 1
      ;;
  esac
  ...
  cleanup_platform_subscribers
  verify_pvc_identity
  "$script_dir/single-ue-lifecycle.sh" validate
  unlink "$state_file"
  printf 'platform_rollback_target_revision=%s\n' "$BASE_REVISION"
  printf 'platform_rollback_result_revision=%s\n' "$rollback_revision"
  printf 'platform_rollback=pass\n'
}
...
run_preflight() {
  ...
  if [[ -e $state_file || -L $state_file ]]; then
    printf 'error: unresolved platform lifecycle state exists; inspect before migration\n' >&2
    return 1
  fi
  ...
  helm template "$release" "$chart" --namespace "$namespace" \
    --kube-version 1.36.1 --values "$profile" > "$render_dir/first.yaml"
  helm template "$release" "$chart" --namespace "$namespace" \
    --kube-version 1.36.1 --values "$profile" > "$render_dir/second.yaml"
  cmp --silent "$render_dir/first.yaml" "$render_dir/second.yaml"
  rm -rf -- "$render_dir"
  trap - EXIT
  printf 'deterministic_platform_chart_render=pass\n'
  printf 'platform_preflight=pass\n'
}

case "$action" in
  preflight) run_preflight ;;
  prepare-secret) require_cluster; prepare_secret ;;
  install) require_cluster; install_platform ;;
  upgrade) require_cluster; controlled_upgrade ;;
  repair-sessions) require_cluster; repair_platform_sessions ;;
  ...
  observe-resources) require_cluster; observe_platform_resources ;;
  ...
  remove-secret)
    require_cluster
    if [[ $(release_platform_enabled) != "false" ]]; then
      printf 'error: refuse to remove the platform Secret while platform is deployed\n' >&2
      exit 1
    fi
    verify_secret
    "${kubectl_namespace[@]}" delete secret "$secret_name"
    printf 'platform_subscriber_secret=removed\n'
    ;;
esac
