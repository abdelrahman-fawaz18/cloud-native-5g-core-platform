#!/usr/bin/env python3
"""Validate a complete resilience campaign and generate reviewed evidence."""
...
class AnalysisError(RuntimeError):
    """Rejected or incomplete resilience campaign evidence."""
...
def svg_grouped(summary: dict[str, Any]) -> str:
    ...
    pieces = [
        ...
        '<title>Median resilience campaign detection, Pod readiness, and service recovery times</title>',
        '<rect width="100%" height="100%" fill="#0f172a"/>',
        '<text x="48" y="38" fill="#f8fafc" font-size="22" font-family="sans-serif">resilience campaign median recovery boundaries</text>',
...
def svg_modes(rows: list[dict[str, Any]]) -> str:
    ...
    pieces = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">',
        '<title>Automatic and operator-assisted resilience campaign service recovery outcomes</title>',
        '<rect width="100%" height="100%" fill="#0f172a"/>',
...
def svg_disruption(summary: dict[str, Any]) -> str:
    ...
    pieces = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">',
        '<title>Median observed user-plane disruption by resilience campaign fault component</title>',
        '<rect width="100%" height="100%" fill="#0f172a"/>',
...
def report_text(campaign: dict[str, Any], summary: dict[str, Any], rows: list[dict[str, Any]]) -> str:
    ...
    lines += [
        ...
        "- Every attempt ended with complete platform and observability stack validation.",
...
def analyze(root: Path) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    experiment_path = root / "benchmarks" / "resilience" / "experiment.json"
    state_path = root / "artifacts" / "kubernetes" / "resilience-campaign.json"
    if not state_path.is_file() or state_path.is_symlink():
        raise AnalysisError("resilience campaign state is absent or unsafe")
    experiment = json.loads(experiment_path.read_text(encoding="utf-8"))
    state = json.loads(state_path.read_text(encoding="utf-8"))
    experiment_sha = hashlib.sha256(experiment_path.read_bytes()).hexdigest()
    if state.get("status") != "raw_complete":
        raise AnalysisError("resilience campaign is not raw_complete")
    ...
    campaign_dir = root / "benchmarks" / "raw" / "resilience" / state["campaign_id"]
...
def write_outputs(root: Path, state: dict[str, Any], rows: list[dict[str, Any]],
    ...
    results = root / "benchmarks" / "resilience" / "results"
    plots = results / "plots"
    ...
    report = root / "reports" / "resilience-results.md"
    report.write_text(
        report_text(summary["campaign"], summary["components"], rows), encoding="utf-8"
    )
    print(f"resilience_analysis_campaign={state['campaign_id']}")
    print("resilience_analysis_conditions=pass accepted=9")
    print("resilience_analysis_outputs=pass csv=2 json=1 svg=3 report=1")
...
def main() -> int:
    ...
    try:
        ...
        print(f"error: resilience campaign analysis rejected the evidence: {error}", file=sys.stderr)
        return 1
    return 0
