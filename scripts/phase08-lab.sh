...
usage() {
  cat <<'EOF'
Usage: sudo ./scripts/phase08-lab.sh ACTION [ARGUMENT]
       ./scripts/phase08-lab.sh analyze

Actions:
  preflight             Validate the accepted Phase 5/6 baseline, resource
  ...
  recover               Run the scoped dependency-ordered recovery path and
                        require complete Phase 5/6 validation.
  status                Show release, fault-target, PVC, campaign, and recent
                        pilot state without changing the cluster.

Raw evidence is permission-restricted and ignored under benchmarks/raw/phase-08.
...
if [[ $action == analyze ]]; then
  exec "$script_dir/analyze-phase08.py" --project-root "$project_root"
fi
...
experiment="$project_root/benchmarks/phase-08/experiment.json"
...
raw_root="$project_root/benchmarks/raw/phase-08"
runner="$script_dir/run-phase08-recovery.py"
campaign_state="$project_root/artifacts/kubernetes/phase-08-campaign.json"
...
require_phase06_baseline() {
  ...
  [[ $status == deployed && $(jq -r '.phase05.enabled' <<<"$values") == true && \
    ...
    $(jq -r '.phase07.enabled // false' <<<"$values") == false ]] || {
    printf 'error: Phase 8 requires the accepted post-rollback Phase 5/6 release without the Phase 7 benchmark overlay\n' >&2
    return 1
  }
  ...
  printf 'phase08_dependency_releases=deployed-and-enabled\n'
}
...
verify_resource_budget() {
  ...
  (( memory_kib >= 4 * 1024 * 1024 )) || {
    printf 'error: Phase 8 preparation requires at least 4 GiB available memory\n' >&2
    return 1
  }
  (( disk_kib >= 8 * 1024 * 1024 )) || {
    printf 'error: Phase 8 preparation requires at least 8 GiB free Docker space\n' >&2
    return 1
  }
  printf 'phase08_resource_budget=pass\n'
}
...
PY
  printf 'phase08_experiment_contract=pass components=amf,smf,upf repetitions=3\n'
}
...
verify_fault_targets() {
  ...
  printf 'phase08_fault_targets=pass replicas=1 selectors=exact\n'
}
...
deterministic_render() {
  ...
  helm lint "$chart" --strict --values "$phase05_values" --values "$phase06_values"
  for suffix in one two; do
    helm template "$release" "$chart" --namespace "$namespace" \
      --kube-version 1.36.1 --values "$phase05_values" \
      --values "$phase06_values" > "$render_dir/$suffix.yaml"
  done
  cmp --silent "$render_dir/one.yaml" "$render_dir/two.yaml"
  rm -rf -- "$render_dir"
  trap - RETURN
  printf 'deterministic_phase08_baseline_render=pass\n'
}
...
run_preflight() {
  ...
  verify_fault_targets
  deterministic_render
  printf 'phase08_preflight=pass\n'
  printf 'next_step=sudo ./scripts/phase08-lab.sh pilot amf\n'
}
...
run_pilot() {
  ...
  case "$component" in
    amf) printf 'next_step=sudo ./scripts/phase08-lab.sh pilot smf\n' ;;
    smf) printf 'next_step=sudo ./scripts/phase08-lab.sh pilot upf\n' ;;
    upf) printf 'next_step=sudo ./scripts/phase08-lab.sh run-matrix\n' ;;
  esac
}
...
test_mongodb() {
  ...
  "$script_dir/phase05-lab.sh" validate > "$run_dir/phase05-validation.log" 2>&1 || result=1
  "$script_dir/phase06-lab.sh" validate > "$run_dir/phase06-validation.log" 2>&1 || result=1
  ...
  printf 'phase08_mongodb_recreation=pass pvc_identity=preserved subscriber_records=5\n'
}
...
test_invalid_config() {
  ...
  if helm template "$release" "$chart" --namespace "$namespace" \
    --values "$phase05_values" --values "$phase06_values" \
    --set phase05.ueReplicas=4 > "$render_output" 2>&1; then
...
kind: Deployment
metadata:
  name: cn5g-phase08-invalid-config
  namespace: cn5g
  labels:
    app.kubernetes.io/part-of: cn5g
    cn5g.io/test: phase08-invalid-config
spec:
  replicas: -1
  selector:
    matchLabels: {app: cn5g-phase08-invalid-config}
  template:
    metadata:
      labels: {app: cn5g-phase08-invalid-config}
    spec:
      containers:
        - name: invalid
...
PY
  if "${kubectl_ns[@]}" apply --server-side --dry-run=server \
    --field-manager=cn5g-phase08-invalid-config --filename "$manifest" \
    > "$api_output" 2>&1; then
    printf 'error: invalid negative-replica Deployment was unexpectedly admitted\n' >&2
    return 1
  fi
  if "${kubectl_ns[@]}" get deployment cn5g-phase08-invalid-config >/dev/null 2>&1; then
  ...
  trap - RETURN
  printf 'phase08_invalid_helm_values=rejected\n'
  printf 'phase08_invalid_kubernetes_object=rejected dry_run=server\n'
  printf 'phase08_invalid_configuration_test=pass release_revision_unchanged=%s\n' "$revision_before"
}
...
recover_baseline() {
  ...
  "$script_dir/phase06-lab.sh" validate
  verify_fault_targets
  printf 'phase08_recovery=pass baseline=phase05-and-phase06\n'
}
