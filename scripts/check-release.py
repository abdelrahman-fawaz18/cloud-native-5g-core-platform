#!/usr/bin/env python3
"""Fail-closed publication, claim, and visual-evidence checks for release qualification."""
...
REPORT = ROOT / "reports" / "release-qualification.md"
EXCLUDED_PARTS = {".git", "artifacts", "migration", "__pycache__"}


class CheckFailure(RuntimeError):
    """A release requirement did not pass."""
...
def check_structure() -> None:
    required = (
        ...
        "docs/runbooks/release-qualification.md",
    ...
    if contract.get("schema_version") != 1:
        raise CheckFailure("release qualification evidence schema_version must be 1")
...
def check_claims() -> None:
    ...
    for claim in contract.get("claims", []):
        ...
        for value in evidence:
            ...
            evidence_count += 1
    print(
        f"release_claim_traceability=pass claims={len(identifiers)} "
...
def check_privacy() -> None:
    ...
    if findings:
        raise CheckFailure("\n".join(findings))
    print(f"release_publication_privacy=pass files={len(files)}")
...
def check_visuals() -> None:
    ...
    print(f"release_visual_evidence=pass captures={len(captures)} roles=4")
...
def check_public_release() -> None:
    ...
    print("release_public_release_gate=pass decision=ready")
