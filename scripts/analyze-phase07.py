#!/usr/bin/env python3
"""Validate and summarize one accepted Phase 7 raw benchmark campaign."""
...
def analyze(root: Path, state_path: Path, experiment_path: Path, output: Path,
    ...
    campaign = root / "benchmarks/raw/phase-07" / campaign_id
    ...
    excluded = []
    for manifest_path in sorted((root / "benchmarks/raw/phase-07").glob("*-matrix/repetition-*/level-*/attempt-*/manifest.json")):
    ...
    grouped_svg(
        ...
        "Phase 7 aggregate traffic results",
    ...
    report = f"""# Phase 7 Performance And Capacity Experiment Report
...
""" + "\n".join(f"- {item}" for item in summary["limitations"]) + "\n"
    ...
    print(f"phase07_analysis_campaign={campaign_id}")
    print("phase07_analysis_conditions=pass accepted=9")
    print("phase07_analysis_outputs=pass csv=3 json=1 svg=3 report=1")
...
def main() -> int:
    ...
    analyze(
        root,
        args.state_file or root / "artifacts/kubernetes/phase-07-campaign.json",
        args.experiment or root / "benchmarks/phase-07/experiment.json",
        args.output or root / "benchmarks/phase-07/results",
        args.report or root / "reports/07_phase07_performance.md",
    )
    return 0
