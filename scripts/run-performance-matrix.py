#!/usr/bin/env python3
"""Run the bounded, resumable performance campaign concurrency experiment."""
...
class MatrixRunner:
    ...
    def wait_for_ues(self, expected: int, timeout_seconds: int = 600) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if converged:
                print(f"performance_ue_statefulset=converged replicas={expected}", flush=True)
                return
            time.sleep(2)
    ...
    def reset_condition_state(self, evidence_dir: Path, label: str) -> None:
        ...
        print(f"performance_condition_reset=started condition={label}", flush=True)
        ...
        self.stream_command(
            [str(self.root / "scripts/platform-lifecycle.sh"), "repair-sessions"], log_path
        )
        self.chown_tree(evidence_dir)
        print(
            f"performance_condition_reset=pass condition={label} "
            ...
            try:
                with urlopen(f"{self.prometheus_url}/-/ready", timeout=2) as response:
                    if response.status == 200:
                        print("performance_prometheus_forward=ready", flush=True)
                        return
            except OSError:
                pass
            time.sleep(1)
    ...
    def run_condition(self, repetition: int, level: int) -> None:
        ...
        if accepted.exists():
            print(f"performance_condition=skipped-already-pass repetition={repetition} level={level}", flush=True)
            return
        ...
        print(f"performance_condition=started repetition={repetition} level={level} attempt={attempt.name}", flush=True)
        ...
        if error is not None:
            print(f"performance_condition=fail repetition={repetition} level={level} raw_evidence={attempt}", file=sys.stderr, flush=True)
            raise CampaignError(str(error)) from error
        write_json(accepted, {"status": "pass", "attempt": attempt.name, "accepted_at": utc_iso()})
        self.chown_tree(condition)
        print(f"performance_condition=pass repetition={repetition} level={level} raw_evidence={attempt}", flush=True)
    ...
    def collect_idle_baseline(self) -> None:
        ...
        if accepted.exists():
            print("performance_idle_baseline=skipped-already-pass", flush=True)
            return
        ...
        started = time.time()
        print(f"performance_idle_baseline=observing seconds={duration}", flush=True)
        time.sleep(duration)
        finished = time.time()
        ...
        print("performance_idle_baseline=pass", flush=True)

    def successful_pilot_exists(self) -> bool:
        image_state = json.loads(
            (self.root / "artifacts/kubernetes/performance-image.json").read_text(encoding="utf-8")
        )
        release = run([
    ...
    def campaign_state(self) -> dict[str, Any]:
        image_state_path = self.root / "artifacts/kubernetes/performance-image.json"
        ...
        if self.state_path.exists():
            ...
            archive = self.state_path.with_name(
                f"performance-campaign-{state['campaign_id']}-abandoned.json"
            )
            ...
            print(
                f"performance_prior_campaign=preserved-abandoned campaign={state['campaign_id']} "
                f"state={archive}",
                flush=True,
            )
    ...
    def execute(self) -> None:
        if not self.successful_pilot_exists():
            raise CampaignError("no successful performance campaign pilot evidence exists")
        ...
        print(f"performance_campaign={state['campaign_id']} mode=resumable", flush=True)
        try:
            ...
            print(f"performance_matrix=raw-complete campaign={state['campaign_id']} raw_evidence={self.campaign_dir}", flush=True)
        finally:
...
def main() -> int:
    try:
        ...
        print(f"error: performance campaign matrix stopped safely: {error}", file=sys.stderr, flush=True)
        return 1
    return 0
