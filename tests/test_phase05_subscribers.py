#!/usr/bin/env python3
"""Behavioral tests for deterministic Phase 5 subscriber generation."""
...
SCRIPT = ROOT / "scripts" / "generate-phase05-subscribers.py"
PLAN = ROOT / "configs" / "kubernetes" / "phase-05" / "subscriber-plan.json"


def load_generator():
    spec = importlib.util.spec_from_file_location("phase05_generator", SCRIPT)
...
class Phase05SubscriberTests(unittest.TestCase):
    ...
    def test_generation_is_byte_deterministic_and_idempotent(self):
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "phase-05"
            ...
            self.assertIn("phase05_subscriber_material_validation=pass", checked.stdout)
