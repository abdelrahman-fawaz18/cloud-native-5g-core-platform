#!/usr/bin/env python3
"""Static safety and version-contract tests for single-UE profile Helm tooling."""
...
class SingleUeProfileToolingStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.manifest = parse_env(ROOT / "versions" / "platform-runtime.env")
    ...
    def test_helm_inputs_are_exact_and_integrity_checked(self):
        ...
        self.assertEqual(self.manifest["SINGLE_UE_STATUS"], "accepted")
        for key in (
    ...
    def test_public_tooling_does_not_embed_local_identity(self):
        for content in (self.installer, (ROOT / "versions" / "platform-runtime.env").read_text()):
