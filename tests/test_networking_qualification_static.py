#!/usr/bin/env python3
"""Static safety and version-contract tests for Networking qualification feasibility tooling."""
...
class NetworkingQualificationStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.manifest = parse_env(ROOT / "versions" / "kubernetes-runtime.env")
        cls.installer = (
            ROOT / "scripts" / "install-kubernetes-tools.sh"
        ).read_text(encoding="utf-8")
        cls.lifecycle = (
            ROOT / "scripts" / "cluster-lifecycle.sh"
        ).read_text(encoding="utf-8")
        cls.cluster_config = (
            ROOT / "configs" / "kind" / "cluster.yaml"
        ).read_text(encoding="utf-8")
        cls.subnet_checker = (
            ROOT / "tools" / "check_kind_subnets.py"
        ).read_text(encoding="utf-8")
        cls.probe_script = (
            ROOT / "scripts" / "networking-qualification.sh"
        ).read_text(encoding="utf-8")
    ...
    def test_binary_inputs_are_exact_and_integrity_checked(self):
        ...
        self.assertEqual(self.manifest["NETWORKING_STATUS"], "accepted")
        for key in (
    ...
    def test_cluster_subnets_do_not_overlap_compose_reference_ranges(self):
        networking = [
        ...
        for candidate in networking:
            for existing in compose_reference:
    ...
    def test_networking_public_scripts_do_not_embed_a_home_path(self):
    ...
    def test_probe_image_reuses_exact_compose_reference_runtime(self):
