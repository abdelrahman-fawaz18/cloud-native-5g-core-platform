#!/usr/bin/env python3
"""Static safety and experiment-contract tests for performance campaign."""
...
OVERLAYS = [ROOT / "profiles" / "performance.yaml"]
DEFAULT_PROFILE = ROOT / "profiles" / "default.yaml"
SCRIPT = ROOT / "scripts" / "performance-campaign.sh"
DOCKERFILE = ROOT / "containers" / "benchmark" / "Dockerfile"
VERSIONS = ROOT / "versions" / "performance-runtime.env"
EXPERIMENT = ROOT / "benchmarks" / "performance" / "experiment.json"
MATRIX_RUNNER = ROOT / "scripts" / "run-performance-matrix.py"
REVIEWED_SUMMARY = ROOT / "benchmarks" / "performance" / "results" / "summary.json"
REVIEWED_METRICS = OBS_CHART / "files" / "performance-reviewed.prom"
METRICS_GENERATOR = ROOT / "scripts" / "generate-performance-dashboard-metrics.py"
...
class PerformanceCampaignStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_benchmark_image_source_and_packages_are_exactly_pinned(self):
        ...
        self.assertIn("PERFORMANCE_IPERF_PORT_COUNT='5'", self.versions)
    ...
    def test_default_and_observability_renders_do_not_gain_benchmark_containers(self):
        observability = helm(
            "template", "cn5g", str(CHART), "--namespace", "cn5g",
            "--kube-version", "1.36.1", "--values", str(DEFAULT_PROFILE),
        )
        self.assertEqual(observability.returncode, 0, observability.stderr)
        self.assertNotIn("benchmark-client", observability.stdout)
        self.assertNotIn("benchmark-server", observability.stdout)
        self.assertNotIn("iperf-tcp", observability.stdout)
    ...
    def test_lifecycle_has_identity_safety_abort_and_restoration_gates(self):
        for required in (
            "performance_local_image=verified", "performance_node_image=verified",
            "apk info -v 2>/dev/null", "grep -Fx 'iperf3-",
            "server_side_performance_dry_run=pass", "performance_benchmark_boundary=pass",
            ...
            "platform-lifecycle.sh\" validate",
            "mongodb_pvc_identity", "performance_rollback=pass",
            "benchmarks/raw/performance", "run-matrix", "run-performance-matrix.py",
        ):
        ...
        self.assertLess(
            ...
            pilot.index('platform-lifecycle.sh" repair-sessions'),
        )
    ...
    def test_reviewed_dashboard_metrics_are_deterministic_bounded_and_sanitized(self):
        ...
        samples = [
            ...
            if line.startswith("cn5g_performance_reviewed_")
        ]
        self.assertEqual(len(samples), 556)
        self.assertLessEqual(len(samples), 600)
        self.assertIn(
            'cn5g_performance_reviewed_campaign_info{campaign_id="20260806T223341Z-matrix",status="reviewed_complete"} 1',
            self.reviewed_metrics,
        )
        self.assertIn("cn5g_performance_reviewed_accepted_conditions 9", self.reviewed_metrics)
    ...
    def test_reviewed_results_exporter_is_project_scoped_and_least_privileged(self):
        deployment = self.obs_object_named(
            "Deployment", "cn5g-observability-performance-results"
        )
        ...
        service = self.obs_object_named(
            "Service", "cn5g-observability-performance-results"
        )
        ...
        self.assertIn("job_name: performance-reviewed-results", scrape)
        self.assertIn("cn5g-observability-performance-results:8080", scrape)
    ...
    def test_performance_dashboard_explains_reviewed_evidence_and_limitations(self):
        ...
        self.assertEqual(set(dashboard["tags"]), {
            "cn5g", "performance", "performance", "reviewed-evidence",
        })
        ...
        self.assertIn("benchmarks/performance/experiment.json", serialized)
        self.assertIn("reports/performance-results.md", serialized)
        self.assertIn("reviewed performance campaign local-lab results", serialized.lower())
        ...
        self.assertIn("cn5g_performance_reviewed_throughput_bits_per_second", expressions)
        self.assertIn("cn5g_performance_reviewed_component_cpu_peak_cores", expressions)
