#!/usr/bin/env python3
"""Static release-readiness and safety-contract tests for release qualification."""
...
LIFECYCLE = ROOT / "scripts" / "release-qualification.sh"
CHECKER = ROOT / "scripts" / "check-release.py"
CONTRACT = ROOT / "release" / "release-evidence.json"
ARCHITECTURE = ROOT / "docs" / "architecture" / "release-qualification.md"
RUNBOOK = ROOT / "docs" / "runbooks" / "release-qualification.md"
VISUAL_RULES = ROOT / "docs" / "images" / "dashboards" / "README.md"
GALLERY = ROOT / "docs" / "dashboard-gallery.md"


class ReleaseQualificationStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_candidate_checker_compiles_and_passes_without_visual_overclaim(self):
        ...
        self.assertIn("release_structure=pass", result.stdout)
        self.assertIn("release_claim_traceability=pass", result.stdout)
        self.assertIn("release_publication_privacy=pass", result.stdout)
    ...
    def test_final_gate_requires_accepted_contract_visuals_and_ready_report(self):
        for required in (
            ...
            "release_public_release_gate=pass decision=ready",
            "release_audit=pass decision=ready",
    ...
    def test_lifecycle_is_scoped_and_never_publishes_or_deletes(self):
        for required in (
            ...
            "release_clean_checkout=pass",
            "release_privileged_gate=pass",
            "release_clean_runtime_targets=reviewed",
            "release_clean_runtime_rebind=pass",
            "merge-base --is-ancestor",
            ".target_review_commit",
            "release_clean_deployment=pass",
            "release_clean_runtime=pass deployment=pass teardown=pass",
            'recreated_node_id != "$source_node_id"',
            '"$clean_runtime_evidence"',
            "release_hosted_gate=pass privileged_validation=not-run",
        ):
    ...
    def test_gallery_links_every_required_visual_role(self):
        ...
        for image in (
            ...
            "performance-reviewed.png",
            "resilience-reviewed.png",
        ):
