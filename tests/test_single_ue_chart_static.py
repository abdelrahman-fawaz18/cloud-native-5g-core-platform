#!/usr/bin/env python3
"""Static rendering, schema, and security tests for the single-UE profile Helm chart."""
...
PROFILE = ROOT / "profiles" / "single-ue.yaml"
...
class SingleUeProfileChartStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        first = helm(
            "template",
    ...
    def test_chart_lints_strictly(self):
        result = helm("lint", str(CHART), "--strict", "--values", str(PROFILE))
    ...
    def test_resource_requests_match_single_ue_observation(self):
        self.assertEqual(
