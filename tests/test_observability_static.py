#!/usr/bin/env python3
"""Static architecture, security, and lifecycle tests for observability stack."""
...
LIFECYCLE = ROOT / "scripts" / "observability-lifecycle.sh"
VERSIONS = ROOT / "versions" / "observability-runtime.env"
...
class ObservabilityStackStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        core = helm(
    ...
    def test_both_charts_lint_strictly_and_render_deterministically(self):
        core_lint = helm(
            "lint", str(CORE_CHART), "--strict", "--values", str(DEFAULT_PROFILE),
        )
    ...
    def test_security_boundary_has_no_privileged_or_host_mounted_workloads(self):
        ...
        self.assertFalse(tokens["cn5g-observability-performance-results"])
        self.assertFalse(tokens["cn5g-observability-resilience-results"])
    ...
    def test_prometheus_covers_platform_telecom_and_bounded_probe_metrics(self):
        ...
        for job in (
            ...
            "kubernetes-cadvisor", "alert-exercise", "performance-reviewed-results",
            "resilience-reviewed-results",
        ):
    ...
    def test_grafana_and_loki_are_fully_provisioned_from_git(self):
        ...
        for path in dashboard_paths:
            ...
            if dashboard["uid"] == "cn5g-performance":
                self.assertIn("performance", dashboard["tags"])
            elif dashboard["uid"] == "cn5g-reliability":
                self.assertIn("resilience", dashboard["tags"])
            else:
                self.assertIn("observability", dashboard["tags"])
    ...
    def test_lifecycle_is_syntax_valid_scoped_and_reversible(self):
        ...
        for expected in (
            "observability_preflight=pass", "server_side_observability_dry_run=pass",
            "observability_validation=pass", "observability_alert_lifecycle=pass tested=3",
            "grafana_provisioning=pass", "metric_cardinality=bounded",
            "observability_uninstall=pass", "observability_destroy=pass",
            "--address 127.0.0.1", "platform-lifecycle.sh repair-sessions",
            "--rollback-on-failure", "alert-exercise|performance-reviewed-results|resilience-reviewed-results|open5gs-",
            ...
            'count({__name__=~"cn5g_performance_reviewed_.+"})',
            "performance_reviewed_metric_validation=pass",
            'count({__name__=~"cn5g_resilience_reviewed_.+"})',
            "resilience_reviewed_metric_validation=pass",
    ...
    def test_stale_state_reset_archives_only_a_proven_old_lineage(self):
        for expected in (
            ...
            "stale-state reset requires the deployed platform topology",
            "stale-state reset requires observability to be absent",
            "refusing stale-state reset with a matching Helm revision",
            'mv -- "$file" "$archive"',
            "observability_stale_state=archived",
        ):
            self.assertIn(expected, self.lifecycle)

    def test_public_observability_artifacts_do_not_embed_local_identity_or_secrets(self):
