#!/usr/bin/env python3
"""Static tests for the single-UE profile synthetic-subscriber material boundary."""
...
TEMPLATE_DIR = ROOT / "configs" / "kubernetes" / "single-ue" / "secret-templates"


class SingleUeProfileSecretStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_runtime_output_is_inside_an_ignored_artifact_directory(self):
        self.assertIn("artifacts/secrets/single-ue", self.script)
        result = subprocess.run(
            ["git", "check-ignore", "-q", "artifacts/secrets/single-ue/ue.yaml"],
            cwd=ROOT,
            check=False,
        )
