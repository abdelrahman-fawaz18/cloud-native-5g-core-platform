#!/usr/bin/env python3
"""Static safety and acceptance tests for the Phase 5 runtime helpers."""
...
LIFECYCLE = ROOT / "scripts" / "phase05-lab.sh"
VALIDATOR = ROOT / "scripts" / "validate-phase05.sh"


class Phase05LifecycleStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_lifecycle_is_exactly_scoped_and_avoids_broad_cleanup(self):
        self.assertIn("cn5g-subscribers-phase05", self.lifecycle)
        self.assertIn("phase-05-upgrade.state", self.lifecycle)
    ...
    def test_preflight_enforces_resources_and_reproducible_inputs(self):
        for expected in (
            "6 * 1024 * 1024", "512 * 1024", "phase05_resource_budget=pass",
            '"$generator" --validate-plan', '"$generator" --check',
            'helm lint "$chart" --strict --values "$overlay"',
            "deterministic_phase05_chart_render=pass",
        ):
            self.assertIn(expected, self.lifecycle)
        self.assertIn("(.phase05.enabled // false)", self.lifecycle)
    ...
    def test_secret_is_file_backed_hash_verified_and_seed_is_excluded(self):
        ...
        self.assertIn("phase05_secret_files", self.lifecycle)
    ...
    def test_deployment_to_statefulset_transition_is_controlled(self):
        ...
        self.assertIn("server_side_phase05_upgrade_dry_run=pass", self.lifecycle)
        self.assertIn("--values \"$overlay\"", self.lifecycle)
        self.assertIn("BASE_REVISION", self.lifecycle)
        self.assertIn("mongodb_pvc_identity=preserved", self.lifecycle)
        self.assertIn("controlled_phase05_upgrade_resume=pre-apply", self.lifecycle)
        self.assertIn(
            "controlled_phase05_upgrade_resume=post-apply-convergence",
            self.lifecycle,
        )
        self.assertIn("deployed|failed|pending-upgrade", self.lifecycle)
        self.assertIn("helm_phase05_upgrade_submission=pass", self.lifecycle)
        self.assertIn("wait_for_phase05_foundation", self.lifecycle)
        self.assertIn("phase05_foundation_readiness=pass", self.lifecycle)
        self.assertIn("verify_ue_statefulset_ownership", self.lifecycle)
        self.assertIn("phase05_ue_statefulset=quiesced", self.lifecycle)
        self.assertIn("phase05_nrf_profile_convergence=pass count=9", self.lifecycle)
        ...
        self.assertLess(
            restart_body.index("scale_phase05_ues 0"),
            restart_body.index("for component in"),
        )
        self.assertLess(
            restart_body.index("wait_for_phase05_nrf_profiles"),
            restart_body.index("scale_phase05_ues 5"),
        )
        ...
        self.assertLess(
            upgrade_body.index("scale_phase05_ues 0"),
            upgrade_body.index('wait_for_phase05_foundation "$new_revision"'),
        )
        self.assertLess(
            upgrade_body.index('wait_for_phase05_foundation "$new_revision"'),
    ...
    def test_rollback_removes_only_phase05_managed_records(self):
        ...
        self.assertIn("expected one restored Phase 4 subscriber", self.lifecycle)
        ...
        self.assertIn("controlled_phase05_rollback_resume=post-apply", self.lifecycle)
        ...
        self.assertIn('"$script_dir/helm-lab.sh" validate', self.lifecycle)
    ...
    def test_resource_observation_covers_each_ue_and_both_dnn_endpoints(self):
        for expected in (
            "observe-resources", "phase05_cgroup_sample",
            "phase05_resource_contract", "/sys/fs/cgroup/cpu.stat",
            ...
            self.assertIn(expected, self.lifecycle)
        observation_body = self.lifecycle.split("observe_phase05_resources()", 1)[1].split(
            "cleanup_phase05_subscribers()", 1
        )[0]
    ...
    def test_validator_proves_five_per_ue_results_and_dnn_isolation(self):
        for expected in (
            ...
            "cross_dnn_isolation=pass", "phase05_validation=pass",
        ):
