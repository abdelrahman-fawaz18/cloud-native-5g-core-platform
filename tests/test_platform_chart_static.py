#!/usr/bin/env python3
"""Static rendering and security tests for the platform Helm topology."""
...
OVERLAY = ROOT / "profiles" / "default.yaml"
SINGLE_UE_PROFILE = ROOT / "profiles" / "single-ue.yaml"
...
class MultiUePlatformChartStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        result = helm(
    ...
    def test_platform_overlay_lints_strictly_and_renders_deterministically(self):
    ...
    def test_platform_object_model_has_five_stable_ue_identities(self):
    ...
    def test_ordinal_selects_exact_matching_secret_files(self):
        ...
        self.assertEqual(secret["secretName"], "cn5g-subscribers")
    ...
    def test_platform_readiness_accepts_both_managed_ue_pools(self):
        ue = self.object_named("StatefulSet", "cn5g-ue")
    ...
    def test_two_dnn_contracts_match_the_public_plan(self):
        sessions = self.values["platform"]["sessions"]
        self.assertEqual(
    ...
    def test_all_rendered_platform_shell_programs_have_valid_posix_syntax(self):
        checked = 0
        for obj in self.objects:
    ...
    def test_platform_secret_is_preexisting_and_authentication_values_are_absent(self):
    ...
    def test_single_ue_profile_remains_available_as_controlled_rollback(self):
        result = helm(
            "template", "cn5g", str(CHART), "--namespace", "cn5g",
            "--kube-version", "1.36.1", "--values", str(SINGLE_UE_PROFILE),
        )
    ...
    def test_schema_rejects_topology_drift(self):
        invalid = (
            "platform.ueReplicas=4",
            "platform.sessions[0].pool=10.99.0.0/24",
            "platform.sessions[1].name=unmanaged",
        )
    ...
    def test_negative_test_manifests_are_scoped_and_least_privileged(self):
        invalid = yaml.safe_load(
            (ROOT / "configs/kubernetes/platform/invalid-ue-pod.yaml")
            .read_text(encoding="utf-8")
        )
        reprovision = yaml.safe_load(
            (ROOT / "configs/kubernetes/platform/reprovision-job.yaml")
            .read_text(encoding="utf-8")
        )
