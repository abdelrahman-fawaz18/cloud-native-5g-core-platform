#!/usr/bin/env python3
"""Static safety and release-contract tests for supply-chain assurance."""
...
RUNBOOK = ROOT / "docs" / "runbooks" / "supply-chain-assurance.md"
REPORT = ROOT / "reports" / "supply-chain-assurance.md"
...
class SupplyChainAssuranceStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_tool_and_action_identities_are_complete_and_immutable(self):
        ...
        for name in (
            "ASSURANCE_DATA_NETWORK_IMAGE_ID", "ASSURANCE_BENCHMARK_IMAGE_ID"
        ):
            self.assertRegex(self.versions[name], r"^sha256:[0-9a-f]{64}$")
        self.assertEqual(
            self.versions["ASSURANCE_DATA_NETWORK_IMAGE_ID"],
            version_values_from(ROOT / "versions" / "compose-runtime.env")[
    ...
    def test_lifecycle_is_fail_closed_and_does_not_publish_or_prune(self):
        for required in (
            "assurance_safe_gate=pass", "assurance_image_gate=pass",
            "assurance_privileged_gate=pass", "gitleaks git", "trivy fs",
            "trivy image", "spdx-json", "kubeconform -strict",
            "conftest test", "platform-lifecycle.sh\" validate",
            "observability-lifecycle.sh\" validate", "registry_push=none",
            "assurance_image_promotion=pass", "rollback-images --confirm",
            "assurance_image_promotion_rollback=pass",
            "assurance_core_image_promotion=already-active upgrade=skipped",
        ):
    ...
    def test_quality_gate_prunes_private_and_ignored_trees(self):
        for path in ('.git', 'artifacts', 'benchmarks/raw', 'migration'):
            self.assertIn(f'-path "$project_root/{path}"', self.lifecycle)
        self.assertIn('-prune -o -type f -name \'*.md\' -print0', self.lifecycle)
    ...
    def test_negative_controls_cover_four_failure_classes(self):
        for required in (
            ...
            "assurance_negative_controls=pass",
    ...
    def test_repository_checker_compiles_and_accepts_current_sources(self):
        ...
        self.assertIn("assurance_workflow_policy=pass", result.stdout)
        self.assertIn("assurance_publication_boundary=pass", result.stdout)
    ...
    def test_docker_sources_and_data_network_privilege_boundary_are_exact(self):
        ...
        rendered = subprocess.run(
            [
                ...
                "--values", str(ROOT / "profiles" / "default.yaml"),
            ],
    ...
    def test_public_assurance_docs_explain_claims_and_rollback_without_private_data(self):
        ...
        for required in (
            ...
            "does not", "platform", "observability stack",
        ):
