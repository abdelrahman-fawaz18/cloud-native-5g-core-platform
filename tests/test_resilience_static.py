#!/usr/bin/env python3
"""Static safety and experiment-contract tests for resilience campaign."""
...
OVERLAYS = [ROOT / "profiles" / "default.yaml"]
EXPERIMENT = ROOT / "benchmarks" / "resilience" / "experiment.json"
LIFECYCLE = ROOT / "scripts" / "resilience-campaign.sh"
RUNNER = ROOT / "scripts" / "run-resilience-matrix.py"
ANALYZER = ROOT / "scripts" / "analyze-resilience.py"
ADR = ROOT / "docs" / "adr" / "0011-resilience-fault-model.md"
METHODOLOGY = ROOT / "docs" / "architecture" / "resilience-engineering.md"
RUNBOOK = ROOT / "docs" / "runbooks" / "resilience-testing.md"
REVIEWED_SUMMARY = ROOT / "benchmarks" / "resilience" / "results" / "summary.json"
REVIEWED_METRICS = OBS_CHART / "files" / "resilience-reviewed.prom"
METRICS_GENERATOR = ROOT / "scripts" / "generate-resilience-dashboard-metrics.py"
...
class ResilienceCampaignStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_lifecycle_is_scoped_resumable_and_fail_closed(self):
        for required in (
            "resilience_preflight=pass", "resilience_fault_targets=pass",
            "resilience-campaign.sh pilot amf", "--mode \"$mode\"",
            "test-mongodb", "test-invalid-config", "dry-run=server",
            "platform-lifecycle.sh\" repair-sessions", "platform-lifecycle.sh\" validate",
            "observability-lifecycle.sh\" validate", "resilience_recovery=pass",
            "mongodb-data-cn5g-mongodb-0", "release_revision_unchanged",
            "benchmarks/raw/resilience",
        ):
    ...
    def test_runner_preserves_fault_identity_and_restores_after_interruption(self):
        ...
        for required in (
            ...
            "resilience_emergency_restoration=started", "emergency_restore",
    ...
    def test_analysis_requires_exact_reviewed_campaign_and_is_sanitized(self):
        ...
        for required in (
            ...
            "user-plane-disruption.svg", "resilience-results.md",
        ):
    ...
    def test_public_methodology_and_runbook_explain_the_real_boundary(self):
        ...
        for required in (
            ...
            "MTTD", "MTTR", "PersistentVolumeClaim", "platform",
            "observability stack", "not high availability",
        ):
    ...
    def test_reviewed_dashboard_metrics_are_deterministic_bounded_and_sanitized(self):
        ...
        samples = [
            ...
            if line.startswith("cn5g_resilience_reviewed_")
        ]
        self.assertEqual(len(samples), 75)
        self.assertLessEqual(len(samples), 100)
        self.assertIn(
            'cn5g_resilience_reviewed_campaign_info{campaign_id="20260807T050635Z-matrix",status="reviewed_complete"} 1',
            self.reviewed_metrics,
        )
        self.assertIn("cn5g_resilience_reviewed_accepted_conditions 9", self.reviewed_metrics)
    ...
    def test_reviewed_results_exporter_is_scoped_and_least_privileged(self):
        deployment = self.obs_object_named(
            "Deployment", "cn5g-observability-resilience-results"
        )
        ...
        service = self.obs_object_named(
            "Service", "cn5g-observability-resilience-results"
        )
        ...
        self.assertIn("job_name: resilience-reviewed-results", scrape)
        self.assertIn("cn5g-observability-resilience-results:8080", scrape)
    ...
    def test_reliability_dashboard_separates_infrastructure_and_service_recovery(self):
        ...
        self.assertEqual(set(dashboard["tags"]), {
            "cn5g", "resilience", "reliability", "reviewed-evidence",
        })
        ...
        self.assertIn("benchmarks/resilience/experiment.json", serialized)
        self.assertIn("reports/resilience-results.md", serialized)
