...
actions build local `cn5g/*:phase09` verification images but do not push them.
The privileged gate reads the existing project cluster and runs the accepted
Phase 5 and Phase 6 validators; it does not create a CI runner or deploy from
GitHub.
...
sudo -v
./scripts/capture-host-state.sh before-phase-09
```

Expected result:

```text
snapshot created: .../artifacts/host-state/before-phase-09
```
...
```bash
./scripts/phase09-lab.sh preflight
./scripts/phase09-lab.sh bootstrap-tools
./scripts/phase09-lab.sh quality
./scripts/phase09-lab.sh manifests
./scripts/phase09-lab.sh scan-repository
./scripts/phase09-lab.sh test-controls
```

The final markers are respectively:

```text
phase09_preflight=pass
phase09_tool_bootstrap=pass tools=11 checksums=verified
phase09_quality_gate=pass
phase09_manifest_gate=pass charts=2 schema=kubernetes-1.36 policy=conftest
phase09_repository_scan=pass secrets=absent severity=high-critical
phase09_negative_controls=pass workflow=unpinned image=floating manifest=privileged secret=synthetic
```
...
```bash
sudo ./scripts/phase09-lab.sh image-gate
```
...
```text
phase09_image_build=pass images=5 registry_push=none
phase09_image_scans=pass images=5 severity=high-critical
phase09_sbom=pass format=spdx-json images=5
phase09_image_gate=pass
```
...
Phase 9 endpoint, loads it into the project-owned kind node, performs a
server-side Helm dry run, and validates Phases 5 and 6. The reviewed benchmark
image remains under `cn5g/benchmark:phase09` because Phase 7 is disabled.
...
```text
phase09_alpine_image_identity=pass images=2
phase09_image_evidence=pass scans=5 sboms=5
server_side_phase09_promotion_dry_run=pass
kubernetes_node_container_metrics=pass
prometheus_target_health=pass
phase06_validation=pass
phase09_image_promotion=pass image=cn5g/data-network:0.1.0
```
...
After Phase 5 and Phase 6 are healthy:

```bash
sudo ./scripts/phase09-lab.sh privileged-gate
```

Expected result:

```text
phase09_privileged_gate=pass scope=local-only report=.../local-privileged-gate.json
```
...
- Failed image scans retain reports under `artifacts/phase-09/scans/images/`.
...
- A failed or rejected data-network promotion retains its exact rollback state
  under ignored, permission-restricted Phase 9 artifacts. After reviewing the
  ...
  Success ends with `phase09_image_promotion_rollback=pass`. The action does
  not remove volumes, namespaces, images, routes, or the cluster.
- A failed live validation uses the existing Phase 5 or Phase 6 scoped
...
- The temporary branch can be abandoned without changing `main`. Ignored
  Phase 9 reports and local verification images may remain safely; no broad
...
```bash
./scripts/capture-host-state.sh after-phase-09
```
