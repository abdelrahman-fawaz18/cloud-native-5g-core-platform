# ADR-0009: Phase 7 Benchmark Traffic Path And Tooling
...
Phase 7 must measure the real synthetic 5G user-plane path. Running a traffic
...
The accepted Phase 5 topology has exactly five synthetic subscriber identities
...
- Add an idle, unprivileged benchmark client sidecar to each UE Pod and an
  ...
  the Phase 7 Helm overlay is enabled.
...
- Keep the accepted five-UE topology as the restoration target after every
  experiment and after Phase 7 rollback.
...
- Alpine 3.22 publishes the selected
  ...
  packages.
- Phase 5 already proves that source-bound HTTP and ICMP use `uesimtun0`, reach
...
Apply only the accepted Phase 5 and Phase 6 value overlays, wait for all
