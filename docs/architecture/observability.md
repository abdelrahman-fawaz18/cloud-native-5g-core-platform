...
The observability stack adds operational visibility without changing the accepted 5G service
...
The current chart provisions four live operational dashboards plus reviewed
performance and resilience dashboards backed by deterministic static metric
exporters. The [dashboard design and evidence standard](dashboard-design.md)
defines how live and historical evidence remain visually and semantically
distinct.
...
available replicas, Pod readiness, Job status, and PVC capability. It does not
...
### Pre-performance campaign Grafana Hardening And Dashboard Model

Interactive use after the original observability stack acceptance exposed a real resource
...
- the complete platform, observability stack, and alert-lifecycle validations.
...
complete platform and observability stack validators and all three alert lifecycles passed.
...
correlation. No performance or recovery result is inserted
...
`pending` and `firing` states. observability stack installs four actionable rules:
...
intentionally absent because this capability has no approved external receiver or
...
```text
Helm release cn5g
└── observability stack UE probe ConfigMap, sidecars, and headless metrics port
...
destroy is separate and removes only the exact observability stack PVCs, Secret, and the
...
Security endpoint, object storage, or long-term metrics backend. observability stack proves
the observability model—not production high availability, retention capacity,
or security certification.

## Runtime Acceptance Results

Observability stack was accepted on 2026-08-05 against the five-UE, two-DNN platform
...
| Regression boundary | The complete five-UE/two-DNN platform validator passed after the telemetry overlay |
...
PromQL can calculate rates from counters that exist, but observability stack does not
generate a fixed offered load or define a throughput/loss experiment. Those
measurements belong to performance campaign, where the traffic generator, sampling window,
