...
The Compose reference baseline proves a complete containerized 5G Standalone (5G SA)
...
ceiling of 32 UEs is not a scale claim; Compose reference validates one UE only.
...
| Use a controlled N6 endpoint | Produces deterministic HTTP/ICMP evidence without depending on external Internet, DNS, or firewall policy | Compose reference does not prove public-network access |
...
| DNN | Data Network Name: selects a session's target data network; `internet` in Compose reference |
| S-NSSAI / SST | Single Network Slice Selection Assistance Information / Slice/Service Type: identifies the requested slice; SST `1` in Compose reference |
...
| Metrics listeners | Selected Open5GS functions | HTTP/TCP 9090 | Exposes metrics endpoints; collection begins in a later observability capability |
...
`tests/test_compose_reference_static.py` checks the most important shared values and
...
before a subscriber registers, so `scripts/validate-compose-reference.sh` separately
...
Compose reference verified meaningful health for all services, one synthetic subscriber,
...
Compose reference does not claim multi-UE scale, multiple DNNs or slices, external
...
remain gated by later measured capabilities.

## Relationship To Networking qualification

The Compose baseline establishes the application and protocol reference that
Networking qualification must preserve. The Kubernetes feasibility spike must independently
...
cluster deletion. A failure in Networking qualification can then be attributed to cluster
