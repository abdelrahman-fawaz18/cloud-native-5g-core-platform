# Continuous Integration And Supply-Chain Security

## Objective And Claim Boundary

Supply-chain assurance turns the repository's manual checks into repeatable release gates. It
...
Materials (SBOMs). The capability improves evidence and prevention; it does not make
...
flowchart LR
    ...
    DR --> PLATFORM
    LG --> PLATFORM["Five-UE and two-DNN validation"]
    PLATFORM --> OBS["Metrics, logs, and dashboards"]
    OBS --> E["Timestamped local evidence"]
...
| Local integration | platform and observability stack validators | five UE sessions, two DNNs, isolation, metrics, logs, dashboards | multi-node resilience or zero downtime |
...
URLs, and SHA-256 checksums in `versions/assurance-toolchain.env`. The three third-party
...
Supply-chain assurance includes four negative controls. It deliberately supplies an unpinned
...
Raw scans and SBOMs are written to ignored `artifacts/supply-chain/`. GitHub keeps
