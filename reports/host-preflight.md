...
**Accepted state:** `READY_FOR_CONTAINER_RUNTIME`
...
capability. The live privileged firewall and Network Address Translation (NAT)
...
| Memory | 15 GiB total; about 10 GiB available during inspection | Adequate for the bounded local topology; observability must be resource-limited |
...
host effects require explicit review before Compose reference.
...
All values are provisional until the Networking qualification packet-path feasibility gate.
...
- Stop a controlled experiment if available memory falls below 2 GiB, swap use
  ...
  threshold.
- Start with one UE in capabilities 2-4 and five UEs in platform. Higher counts require
  gradual performance campaign measurement.
...
compatibility, licensing, and the relevant capability tests pass.
...
| MongoDB | `8.0.28` candidate | Verify official container tag, digest, license, and Open5GS compatibility in Compose reference |
| Prometheus | `3.13.0` candidate | Defer final image digest and compatibility acceptance to observability stack |
| Grafana | `13.1.0` candidate | Defer final image digest and compatibility acceptance to observability stack |
| Loki | `3.7.2` candidate | Defer final image digest and compatibility acceptance to observability stack |
| Grafana Alloy | `1.18` series candidate | Use instead of end-of-life Promtail; pin exact image/digest in observability stack |
...
## Expected Host Impact Of Containerization
...
Compose reference should therefore:
...
| Project definition/1 requirement | State |
| --- | --- |
...
## Final Project definition/1 Decision

Project definition and Host baseline pass. No known host prerequisite or conflict blocks the
container-runtime capability. The following conditions remain mandatory rather than
optional:
...
- stop after the Compose reference container/Compose gate rather than creating a
