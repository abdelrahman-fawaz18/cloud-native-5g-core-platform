...
This directory contains synthetic, environment-independent Open5GS,
...
`compose/` contains the Compose reference single-UE baseline: separate Open5GS Network
...
`kubernetes/platform/subscriber-plan.json` is the public, non-secret source of
...
local path. `scripts/generate-subscribers.py` validates it before
deriving ignored runtime material from a local seed.

`kubernetes/platform/invalid-ue-pod.yaml` is an exact, short-lived
...
`kubernetes/platform/reprovision-job.yaml` is an independently owned,
